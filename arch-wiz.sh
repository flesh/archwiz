...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "BACKUP-DESC"  "Backup all Configuration Files from Hard Drive to Flash." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
backup()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "INSTALL-BASE-SYSTEM-DESC"        "Install Base System" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "INSTALL-BASE-SYSTEM-BASE-SYSTEM" "INSTALL BASE SYSTEM" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "INSTALL-BASE-SYSTEM-MSG-1"       "Using the pacstrap script we install the base system. The base-devel package group will be installed also." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "INSTALL-BASE-SYSTEM-COMPLETE"    "INSTALL BASE SYSTEM Completed..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "INSTALL-BASE-SYSTEM-ERROR"       "INSTALL BASE SYSTEM: pacstrap Failed!" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
install_base_system()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CONFIGURE-AUR-HELPER-DESC"          "Configure AUR Helper" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-AUR-HELPER-NOTES"         "Should only be run from Live Mode." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "CONFIGURE-AUR-HELPER-CONFIG-HELP"   "Configuring AUR Helper" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-AUR-HELPER-NOT-INSTALLED" "Not Installed" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-AUR-HELPER-INSTALLED"     "Installed" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CONFIGURE-SUDO-DESC"  "Configure sudo" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-SUDO-NOTES" "Called from Boot Mode." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "CONFIGURE-SUDO-COMPLETE" "Sudo Configured" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-SUDO-SUDO" "SUDO" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
configure_sudo()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "RUN-TASK-MANAGER-DESC"  "Run Task" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #   
    localize_info "RUN-TASK-MANAGER-ERROR" "Error in run_task_manager..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "RUN-TASK-MANAGER-INFO"  "Install TASK MANAGER..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
run_task_manager()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "WRITE SECRET-DESC"  "Write Secret" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi    
# -------------------------------------    
write_secret()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-SECRET-DESC"  "Get Secret" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi    
# -------------------------------------
get_secret()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "DELETE-SECRET-DESC"  "Delete Secret" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
delete_secret()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CREATE-SCRIPT-LIB-DESC"  "Create Script to run in arch-chroot" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
create_script_lib()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CREATE-INSTALL-SCRIPTS-DESC"  "Create Script Boot, which gets run in arch-chroot to install bootloader" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
create_script_boot()
{
    ...
    if [[ "$IS_CUSTOM_NAMESERVER" -eq 1 ]]; then
        ...
        echo "make_dir '/etc/conf.d'" >> ${MOUNTPOINT}/install_scripts
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CREATE-SCRIPT-LOG-DESC"  "Create Script Log" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
create_script_log()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "RUN-INSTALL-SCRIPTS-DESC"  "Run install scripts" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "RUN-INSTALL-SCRIPTS-TITLE" "Run Install Scripts in arch-chroot..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "RUN-INSTALL-SCRIPTS-SUDOERS" "Configure sudo..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "RUN-INSTALL-SCRIPTS-MSG-1" "If all went right you should be able to reboot into a fully functioning Desktop." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "RUN-INSTALL-SCRIPTS-MSG-2" "Make sure to check the root for install files like install_scripts, install_scripts_root_secrets, install_scripts_user_secrets, you can also delete /boot/grub_uefi.log." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
run_install_scripts()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GDISK-PARTITION-DESC"  "Format Hard Drive and setup Partitions." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #   
    localize_info "GDISK-PARTITION-TITLE"       "Partition Setup..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GDISK-PARTITION-SIMULATE"    "Simulate Installation." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GDISK-PARTITION-ERASING"     "Erasing Disk..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GDISK-PARTITION-CREATE-UEFI" "Creating UEFI Disk..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GDISK-PARTITION-CREATE-BIOS" "Creating BIOS Disk..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GDISK-PARTITION-CREATE-BOOT" "Creating BOOT disk Partition ..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GDISK-PARTITION-CREATE-SWAP" "Creating SWAP Partition ..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GDISK-PARTITION-CREATE-ROOT" "Creating ROOT Partition ..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GDISK-PARTITION-CREATE-HOME" "Creating HOME disk Partition ..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GDISK-PARTITION-CREATE-VAR"  "Creating VAR disk Partition ..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GDISK-PARTITION-FORMAT-ROOT" "Formating ROOT Partition..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GDISK-PARTITION-FORMAT-HOME" "Formating HOME disk Partition..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GDISK-PARTITION-FORMAT-VAR"  "Formating VAR disk Partition..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GDISK-PARTITION-FORMAT-BOOT" "Formating BOOT disk Partition..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GDISK-PARTITION-FORMAT-UEFI" "Formating UEFI disk..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GDISK-PARTITION-FORMAT-BIOS" "Formating BIOS disk..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GDISK-PARTITION-MAKE-SWAP"   "Creating SWAP Partition..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
gdisk_partition()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "SETUP-OS-DESC"  "Setup OS" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "SETUP-OS-INSTALL-ARCH"           "Install Arch Linux on Drive" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-GPT"                    "This Script uses GPT: GUID Partition Table (GPT) is a new style of partitioning which is part of the Unified Extensible Firmware Interface Specification, using the globally unique identifier for devices. It is different from the Master Boot Record (the more commonly used partitioning style) in many aspects and has many advantages." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-PARITION-HAR-DRIVE"     "Partitioning a hard drive allows one to logically divide the available space into sections that can be accessed independently of one another. Partition information is stored within a hard drive's Master Boot Record." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-RUNNING-GDISK"          "Running gdisk options will DESTROY all DATA on installation disk!" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-4-PARTITION-TYPES"      "This function sets up a Standard Partition of 4 Primary Types: UEFI - BOOT - SWAP and ROOT; you can also specify to create HOME and VAR Partitions." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-UEFI"                   "Unified Extensible Firmware Interface (or UEFI for short) is a new type of firmware that was initially designed by Intel (known as EFI then) mainly for its Itanium based systems. It introduces new ways of booting an OS that is distinct from the commonly used 'MBR boot code' method followed for BIOS systems. It started as Intel's EFI in versions 1.x and then a group of companies called the UEFI Forum took over its development from which it was called Unified EFI starting with version 2.0." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-UEFI-FAT32"             "UEFI is formated as FAT32" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-BIOS"                   "If your motherboard supports UEFI Mode, then use it, otherwise use SETUP-OS-BIOS mode; both use GPT for this Script." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-BIOS-INFO"              "BIOS is formated as FAT32" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-BOOT"                   "The /boot directory contains the kernel and ramdisk images as well as the bootloader configuration file and bootloader stages." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-BOOT-PART-NOT-REQUIRED" "BOOT is not required to be a separate Partition, its recommended, BOOT is formated as Ext2" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-ROOT"                   "ROOT is required, and is recommended to be formated as Ext4." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-SWAP"                   "The swap partition provides memory that can be used as virtual RAM. It is recommended for PCs with 1GB or less of physical RAM." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-SWAP-INFO"              "A SWAP partition is not required, but recommended." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-HOME"                   "The /home directory contains user-specific configuration files (the so-called 'dot files'). Optionally, it can also hold any type of media (videos, music, etc), and if you use Wine, the games/programs will be installed in ~/.wine/ by default. So please take this into account if you chose to create a separate home partition. While keeping it on a separate partition can be useful in case you reinstall, some prefer to start fresh (because that's usually the reason for a reinstall), instead of reusing old, and possibly deprecated or problematic, configuration files. The main advantage is that, in very rare cases, if the root partition becomes too full, it will not impact your web browser, media player, torrent client, etc. They will keep working uninhibited, and will keep saving new changes to their setting files or to their cache. A home partition can also be shared with other installed Linux distributions, but this is not recommended because of possible incompatibilities between user-specific configuration files. The only exception is if each distribution has its own user dir on the shared home partition." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    ...
    localize_info "SETUP-OS-QUESTIONS"              "A series of question will guild us to setting up the Partition Table." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-PARITION-ORDER"         "The Order the Partitions are made are: UEFI - BOOT - SWAP - ROOT - HOME - VAR" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-SETUP-INFO"             "You can hit Shift-Page Up or Down to scroll screens, Ctrl-C to exit script at any time." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-INSTALL-DRIVE"          "You must choose a Drive to install OS on, make sure you wish to Format this Whole Drive." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-WHAT-DRIVE"             "Figure out what drive you wish to install to, then re-run this Script." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-FORMAT-DRIVE"           "About to Format Drive, be very sure you wish to continue! Hit Ctrl-C to Cancel, any key to Continue." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-LAST-CHANCE"            "Last Chance at line" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-SAME-DRIVE-SCRIPT"      "You can not install to same drive Script is Executing from." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-INSTALL-ON-DEVICE"      "Installing on Device" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SETUP-OS-DEVICE-NO-EXIST"        "Device does not exist! Try running script again using correct device name, run sgdisk -p for a list of Device names." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
setup_os()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "START-SCREEN-DESC"  "Start Screen" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "root-privileges" "This script must be run with root privileges" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "START-SCREEN-TITLE" "Arch Installation" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "START-SCREEN-INFO-1" "The Arch Install Script are a set of Bash scripts that simplify Arch installation." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "START-SCREEN-INFO-2" "This program was Originally Created by helmuthdu mailto: helmuthdu[at]gmail[dot]com called Archlinux Ultimate Install or AUI" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "START-SCREEN-INFO-3" "And refactored by and new functionally written by Jeffrey Scott Flesher, so its more like a Wizard, and its called: ($ARCH_WIZ_SCRIPT_NAME), it allows the user to chose to install from a menu driven selection, then save it so you can load it next time, making this a one stop custom installer, you can edit all your configuration files, save changes to flash drive, and reuse them to install on someone elses machine." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "START-SCREEN-INFO-4" "This Script will write Configuration files on mounted Flash Drive, its up to the user to remove them after Script finishes, so you must have enough space on device to hold files." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "START-SCREEN-INFO-5" "This Script will also write a log file on mounted Flash Drive." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "START-SCREEN-WARN-1" "This script is still in $SCRIPT_STAGE Stage" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "START-SCREEN-WARN-2" "Warning: This script will completely format the Target Drive, its not designed to preserve partitions, it ERASES THE WHOLE HARD DRIVE!. so you have been warned." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "START-SCREEN-INFO-6" "If you are installing this to a dual boot Windows System (or any other OS) on the same drive, it will be lost, which in the case of Windoze, will not be all that much of a lost, but in future version of this script, we will work on converting to GPT Disk and preserving Partitions, and mounting them in the fstab and grub." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
start_screen()
{
