#!/bin/bash
#
declare LAST_UPDATE="27 Feb 2013 16:33"
...
if [[ "$RUN_TEST" -eq 1 ]]; then
    ...
    MyReturn="$?"
    if [[ "$MyReturn" == '6' ]]; then
        echo -e "\t${BBlue}$(gettext -s "TEST-FUNCTION-PASSED") string_len @ $(basename $BASH_SOURCE) : $LINENO${White}"
    else
        echo -e "\t${BRed} $(gettext -s "TEST-FUNCTION-FAILED") string_len @ $(basename $BASH_SOURCE) : $LINENO${White}"
    fi
fi
#}}}
# -----------------------------------------------------------------------------
...
if [[ "$RUN_HELP" -eq 1 ]]; then
    NAME="sub_string"
    USAGE=$(gettext -s "SUB-STRING-USAGE")
...
sub_string()
{
    ...
    local endding=$((beginning+lenght))
    if [[ "$RUN_TEST" -eq 3 ]]; then
        local stringlenght=${#1}
        echo "-------------------------------"
        echo "$3. Passed in="
        echo "[$1]"
        echo "[$1]"
        echo "|1234567890123456789012345678901234567890123456789"
        echo "Lenght=$stringlenght"
        echo "[$2]"
        echo "|1234567890123456789012345678901234567890123456789"
        echo "Lenght=$lenght"
    fi
    if [[ "$3" -eq 1 ]]; then   # Beginning: 'Beginning Search Endding'
        ...
        if [[ "$RUN_TEST" -eq 3 ]]; then
            ...
            echo "|${1:0:$beginning}|"
            echo "MyString=\${1:0:\$beginning}"
        fi
    elif [[ "$3" -eq 2 ]]; then # End: 'Beginning Search Endding'
        MyString="${1:$endding}"
        if [[ "$RUN_TEST" -eq 3 ]]; then
            echo "------"
            echo "|$1|"
            echo "|1234567890123456789012345678901234567890123456789"
            echo "endding=$endding"
            echo "|${1:$endding}|"
            echo "MyString=\${1:\$endding}"
        fi
    elif [[ "$3" -eq 3 ]]; then # Remove: 'Beginning Search Endding'
        MyString="${1:0:$beginning}${1:$endding}"
        if [[ "$RUN_TEST" -eq 3 ]]; then
            echo "MyString=\${MyStringBegin}\${MyStringEnd}"
            echo "-------------------------------"
        fi
    fi    
}
...
if [[ "$RUN_TEST" -eq 1 || "$RUN_TEST" -eq 2 || "$RUN_TEST" -eq 3 ]]; then
    ...
    MyString="$MyStringOriginal"
    sub_string "$MyString" "$MySeperator" 1
    if [[ "$MyString" == "$MyStringBeginning" ]]; then
        echo -e "\t${BBlue}$(gettext -s "TEST-FUNCTION-PASSED") sub_string [$MyStringOriginal] [$MySeperator] 1 <- [${MyString}] @ $(basename $BASH_SOURCE) : $LINENO${White}"
    else
        echo -e "\t${BRed} $(gettext -s "TEST-FUNCTION-FAILED") sub_string [$MyStringOriginal] [$MySeperator] 1 <- [${MyString}] @ $(basename $BASH_SOURCE) : $LINENO${White}"
    fi
    MyString="$MyStringOriginal"
    sub_string "$MyString" "$MySeperator" 2
    if [[ "$MyString" == "$MyStringEnd" ]]; then
        echo -e "\t${BBlue}$(gettext -s "TEST-FUNCTION-PASSED") sub_string [$MyStringOriginal] [$MySeperator] 2 <- [${MyString}] @ $(basename $BASH_SOURCE) : $LINENO${White}"
    else
        echo -e "\t${BRed} $(gettext -s "TEST-FUNCTION-FAILED") sub_string [$MyStringOriginal] [$MySeperator] 2 <- [${MyString}] @ $(basename $BASH_SOURCE) : $LINENO${White}"
    fi
    MyString="$MyStringOriginal"
    sub_string "$MyString" "$MySeperator" 3
    if [[ "$MyString" == "${MyStringBeginning}${MyStringEnd}" ]]; then
        echo -e "\t${BBlue}$(gettext -s "TEST-FUNCTION-PASSED") sub_string [$MyStringOriginal] [$MySeperator] 2 <- [${MyString}] @ $(basename $BASH_SOURCE) : $LINENO${White}"
    else
        echo -e "\t${BRed} $(gettext -s "TEST-FUNCTION-FAILED") sub_string [$MyStringOriginal] [$MySeperator] 2 <- [${MyString}] @ $(basename $BASH_SOURCE) : $LINENO${White}"
    fi
...
localize_save()
{
    ...
    for (( i=0; i<${total}; i++ )); do
        ...
        echo "# ${LOCALIZE_COMMENT[$i]} -> ${LOCALIZE_MSG[$i]}" >> "${LOCALIZED_PATH}/${DEFAULT_LOCALIZED_LANG}/${DEFAULT_LOCALIZED_LANG}.po" # Comment
        echo "msgid \"${LOCALIZE_ID[$i]}\""                     >> "${LOCALIZED_PATH}/${DEFAULT_LOCALIZED_LANG}/${DEFAULT_LOCALIZED_LANG}.po" # Message ID
        echo "msgstr \"${LOCALIZE_MSG[$i]}\""                   >> "${LOCALIZED_PATH}/${DEFAULT_LOCALIZED_LANG}/${DEFAULT_LOCALIZED_LANG}.po" # Message String
    ...
    #        
    TRANSLATOR="simulate" # moses google
    local -a TransLate=( "af" "ca" "de" "es" "fr" "ga" "gl" "hi" "it" "lt" "lv" "mt" "nl" "pl" "pt" "sq" )
    if [[ "$TRANSLATOR" == "simulate" ]]; then
        TransLate=( "af" "ca" "de" "es" "fr" "ga" "gl" "hi" "it" "lt" "lv" "mt" "nl" "pl" "pt" "sq" )
    elif [[ "$TRANSLATOR" == "google" ]]; then
        TransLate=( $(load_2d_array "${FULL_SCRIPT_PATH}/gtranslate-cc.db" "1" ) ) # 1 is for Code
        ...
        echo "set URL"
        TransLate=( "af" "ca" "de" "es" "fr" "ga" "gl" "hi" "it" "lt" "lv" "mt" "nl" "pl" "pt" "sco" "sq" )
    ...
    for (( index=0; index<${transtotal}; index++ )); do
        ...
        for (( i=0; i<${total}; i++ )); do
            ...
            if ! is_string_in_file "$LocalePath" "msgid \"${LOCALIZE_ID[$i]}\"" ; then
                ...
                #
                echo "# ${LOCALIZE_COMMENT[$i]} -> ${LOCALIZE_MSG[$i]}" >> "$LocalePath" # Comment
                echo "msgid \"${LOCALIZE_ID[$i]}\""                     >> "$LocalePath" # Message ID
                echo "msgstr \"$RETURN_TRANS}\""                        >> "$LocalePath" # Message
                echo ""                                                 >> "$LocalePath" # Empty Line
            fi    
        done
        msgfmt -o "${LOCALIZED_PATH}/${TransLate[$index]}/LC_MESSAGES/${LOCALIZED_FILE}.mo" "${LOCALIZED_PATH}/${TransLate[$index]}/${TransLate[$index]}.po"
    done
...
localize_info()
{
    ...
    [[ "$progresion" -ge 3 ]] && progresion=0
    #    
    if [[ "$#" -ne "3" ]]; then echo -e "${BRed}$(gettext -s "WRONG-NUMBER-ARGUMENTS-PASSED-TO")${BYellow} |$@| ${BWhite} -> $FUNCNAME @ $(basename $BASH_SOURCE) : $LINENO ${White}"; exit 1; fi
    [ -z "$1" ] && return 1
    [ -z "$2" ] && return 1
    #echo ">: $1"
...
{ 
    ...
    tput sgr0
    MyString="$(localize "$1")"
    if $(is_needle_in_haystack ": " "$MyString" 4) ; then # 6=Anywhere Exactly
        if [ "$#" -eq "1" ]; then
            sub_string "$MyString" ":" 1
            local Beginning="$MyString"
            sub_string "$(localize "$1")" ":" 2 # MyString was modified so use original
            echo -e "${BYellow}${Beginning}: ${BWhite}${MyString}${White}" | fold -sw $(( $T_COLS - 18 )) | sed 's/^/\t/'
        else
            sub_string "$MyString" ":" 1
            local Beginning="$MyString"
            sub_string "$(localize "$1")" ": " 2 # MyString was modified so use original
            echo -e "${BYellow}${Beginning}: ${BWhite}${MyString}${White} ${2}${White}" | fold -sw $(( $T_COLS - 18 )) | sed 's/^/\t/'
        fi
    else
        if [ "$#" -eq "1" ]; then
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "PRINT-THAT-USAGE" "print_that 1->(Localized Text ID) 2->(Optional Not Localized Text)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PRINT-THAT-DESC"  "Like print_info, without a blank line and indented." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PRINT-THAT-NOTES" "Localized." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
print_that()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "PRINT-INFO-USAGE" "print_caution 1->(Localized Text ID) 2->(Optional Not Localized Text)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PRINT-INFO-DESC"  "Prints information on screen for end users to read, in a Column that is as wide as display will allow." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PRINT-INFO-NOTES" "Localized." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
print_caution()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "PRINT-WARNING-USAGE" "print_warning 1->(Localized Text ID) 2->(Optional Not Localized Text)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PRINT-WARNING-DESC"  "Print Warning" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PRINT-WARNING-NOTES" "Localized." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
print_warning()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "PRINT-TEST-DESC"  "Print Test" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PRINT-TEST-NOTES" "Localized." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
print_test()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "PRINT-ERROR-DESC"  "Print error" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PRINT-ERROR-NOTES" "Localized." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
print_error()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "CHECK-BOX-USAGE" "checkbox 1->(0={ }, 1={X}, 2={U})" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CHECK-BOX-DESC"  "Display {X} or { } or {U} in Menus." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CHECK-BOX-NOTES" "Used in Menu System." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
checkbox()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "CHECKBOX-PACKAGE-DESC"  "check if {X} or { }" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CHECKBOX-PACKAGE-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
checkbox_package()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "CONTAINS-ELEMENT-DESC"  "Array Contains Element" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONTAINS-ELEMENT-NOTES" "Used to Search Options in Select Statement for Valid Selections." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
contains_element()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "INVALID-OPTION-DESC"  "Invalid option" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "INVALID-OPTION-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "INVALID-OPTION-TEXT-1" "Invalid option" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
invalid_option()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "INVALID-OPTIONS-DESC"  "Invalid options" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "INVALID-OPTIONS-NOTES" "Idea was to show all valid options, still in work.." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
invalid_options()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "ASSERT-DESC"  "assert for debugging variables" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ASSERT-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "DEBUGGER-USAGE" "debugger 1->(1=On, x=Off)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DEBUGGER-DESC"  "Add Option: Ran from Task Manager." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DEBUGGER-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
debugger()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "GET-NETWORK-DEVICE-DESC"  "Get Network Devices." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-NETWORK-DEVICE-NOTES" "Holds IP Address if its an Active connection; no test of Internet Access are done." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_network_devices()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "SHOW-USERS-DESC"  "Show Users." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-USERS-NOTES" "Shows users in /etc/passwd." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
show_users()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "DEVICE-LIST-DESC"  "Get Device List." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DEVICE-LIST-NOTES" "Used to get Hard Drive Letter, assumes you are running this from a Flash Drive." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
device_list()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "UMOUNT-PARTITION-DESC"  "Umount partition." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "UMOUNT-PARTITION-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
umount_partition()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "IS-BREAKABLE-DESC"  "is breakable checks to see if key input meets exit condition." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-BREAKABLE-NOTES" "Used to break out of Loops." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
is_breakable() 
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "TO-LOWER-CASE-DESC"  "Make all Lower Case." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "TO-LOWER-CASE-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
to_lower_case()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "TO-UPPER-CASE-DESC"  "Make all Upper Case." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "TO-UPPER-CASE-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
to_upper_case()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "PRINT-ARRAY-USAGE" "print_array 1->(array{@})" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PRINT-ARRAY-DESC"  "Print Array; normally for Troubleshooting; but could be used to print a list." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PRINT-ARRAY-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PRINT-ARRAY-WARN"  "Passed in empty array from" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
print_array()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    # READ INPUT {{{
    localize_info "READ-INPUT-DESC"  "read keyboard input." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-NOTES" "Sets Variable OPTION as return; do not us in AUTOMAN or INSTALL_WIZARD Mode." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
read_input()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "GET-INPUT-OPTION-USAGE"   "get_input_option 1->(array[@] of options) 2->(default) <- return value OPTION" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-INPUT-OPTION-DESC"    "Get Keyboard Input Options between two numbers." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-INPUT-OPTION-NOTES"   "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-INPUT-OPTION-CHOOSE"  "Choose a number between 1 and " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-INPUT-OPTION-DEFAULT" "Default is" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_input_option()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "CLEAN-INPUT-USAGE"   "clean_input 1->(Input String) <- return " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CLEAN-INPUT-DESC"    "Clean Keyboard Input Options:  String of values: 1 2 3 or 1-3 or 1,2,3, replaces Commas with spaces, only allows Alphanumeric, - and space." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CLEAN-INPUT-NOTES"   "Only Allows: Alphanumeric, 1 space between them, converts commas to spaces, converts all Alpha's to lower case." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
clean_input()
{
    # CleanInput='1-3,4,5 6 A B C ^!@*'
    # CleanInput=$(echo -n "$CleanInput" | tr ',' ' ')
    local CleanInput="$1"                            # Assign
    CleanInput=$(trim "$CleanInput")                 # Trim
    CleanInput=$(echo -n "$CleanInput" | tr ',' ' ') # Replace any Commas with space
    CleanInput=${CleanInput//[^a-zA-Z0-9 -]/}        # Clean out anything that's not Alphanumeric, comma or a space
    CleanInput=$(echo -n $CleanInput | tr A-Z a-z)   # Lowercase with TR
    echo "$CleanInput"
}
# -------------------------------------
if [[ "$RUN_TEST" -eq 2 ]]; then
    ...
    if [[ "$MyString" == '1-3 4 5 6 a b c' ]]; then
        echo -e "\t${BBlue}$(gettext -s "TEST-FUNCTION-PASSED")  clean_input @ $(basename $BASH_SOURCE) : $LINENO${White}"
    else
        echo -e "\t${BRed} $(gettext -s "TEST-FUNCTION-FAILED")  clean_input @ $(basename $BASH_SOURCE) : $LINENO${White}"
    fi    
fi
#}}}
# -----------------------------------------------------------------------------
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "READ-INPUT-OPTIONS-USAGE"          "read_input_options 1->(options) 2->(Breakable Key) <- return Array Values in OPTIONS, expanded, duplicates removed and lower case." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-DESC"           "Read Keyboard Input Options:  String of values: 1 2 3 or 1-3 or 1,2,3" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-NOTES"          "AUTOMAN, INSTALL_WIZARD and BYPASS to easily configure default values, hit 'r' to run Recommended Options." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-OPTIONS"        "Use Options" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-IT-1"           "read_input_options Return Array" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-IT-2"           "read_input_options Removed Duplicates" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-VERIFY"         "Verify Input" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-INVALID-NUMBER" "Invalid Number Option: Legal values are" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-INVALID-ALPHA"  "Invalid Alpha Option: Legal values are" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
read_input_options()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "READ-INPUT-YN-USAGE" "read_input_yn 1->(Question) 2->(None Localize) 3->(0=No, 1=Yes) <- return value YN_OPTION" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-YN-DESC"  "Read Keyboard Input for Yes and No." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-YN-NOTES" "Localized." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "Wrong-Key-Yn" "Wrong Key, (Y)es or (n)o required." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "Wrong-Key-Ny" "Wrong Key, (y)es or (N)o required." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
read_input_yn()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "READ-INPUT-DEFAULT-USAGE" "read_input_default 1->(Prompt) 2->(Default Value) <- returns string in OPTION" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-DEFAULT-DESC"  "Read Keyboard Input and allow Edit of Default value." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-DEFAULT-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
read_input_default()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "READ-INPUT-DATA-USAGE" "read_input_data 1->(Localized Prompt) <- return value OPTION" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-DATA-DESC"  "Read Data." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-DATA-NOTES" "Return value in variable OPTION; not to be used in AUTOMAN or INSTALL_WIZARD Mode, since there is no default value." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
read_input_data()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "VERIFY-INPUT-DEFAULT-DATA-USAGE"     "verify_input_default_data 1->(Prompt) 2->(Default-Value) 3->(Default 1=Yes or 0=No) <- return value in OPTION" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-INPUT-DEFAULT-DATA-DESC"      "Verify Keyboard Input of Default Editable Value." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-INPUT-DEFAULT-DATA-NOTES"     "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "VERIFY-INPUT-DEFAULT-DATA-ENTER"     "Enter" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-INPUT-DEFAULT-DATA-VERIFY"    "Verify" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-INPUT-DEFAULT-DATA-NOT-EMPTY" "Can not be empty" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "VERIFY-INPUT-DATA-USAGE"  "verify_input_data 1->(Prompt) 2->(Data) <- return value OPTION" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-INPUT-DATA-DESC"   "verify input data." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-INPUT-DATA-NOTES"  "Localized." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "VERIFY-INPUT-DATA-ENTER"  "Enter" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-INPUT-DATA-VERIFY" "Verify" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-INPUT-DATA-EMPTY"  "Can not be empty" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
verify_input_data()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "IS-OS-USAGE" "is_os 1->(os-name)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-OS-DESC"  "Check to see if OS matches what you think it is" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
is_os()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "IS-WILDCARD-FILE-DESC"  "Test for Files: is_wildcard_file '/from/path/' 'log' # if &lowast;.log exist." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-WILDCARD-FILE-NOTES" "filter: if ' ' all, else use extension, do not pass 'Array' in &lowast; as wildcard. If looking for a '/path/.hidden' file, a /path/&lowast; fails, so use no wild card, i.e. /path/." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
is_wildcard_file()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "MAKE-DIR-DESC"  "Make Directory." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "MAKE-DIR-NOTES" "return 0 if dir created." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
make_dir()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "COPY-FILE-DESC"  "Copy File." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "COPY-FILE-NOTES" "Creates Destination Folder if not exist. LINENO is for Logging and Debugging." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
copy_file()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "COPY-FILES-USAGE" "copy_files 1->(/full-path/) 2->(ext or Blank for *) 3->(/full-path/to_must_end_with_a_slash/) 4->(Debugging Information)<br />${HELP_TAB}copy_files 1->(/full-path/) 2->( ) 3->(/full-path/to_must_end_with_a_slash/) 4->(Debugging Information)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "COPY-FILES-DESC"  "Creates Destination Folder if not exist. LINENO is for Logging and Debugging." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "COPY-FILES-NOTES" "If looking for a '/path/.hidden' file, a /path/&lowast; fails, so use no wild card, i.e. /path/" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
copy_files()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "COPY-DIRECTORY-USAGE" "copy_dir 1->(/full-path/) 2->(/full-path/to_must_end_with_a_slash/) 3->(Debugging Information)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "COPY-DIRECTORY-DESC"  "Copy Directory." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "COPY-DIRECTORY-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "COPY-DIRECTORY-PATH"  "Empty Path." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "COPY-DIRECTORY-COPY"  "Copied Directory" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "COPY-DIRECTORY-ERROR" "Failed to copy Directory." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "COPY-DIRECTORY-MAKE"  "Failed to Make Directory." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
copy_dir()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "REMOVE-FILE-USAGE"     "remove_file 1->(/full-path/from.ext) 2->(Debugging Information)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-FILE-DESC"      "Remove File if it exist." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-FILE-NOTES"     "if -f > rm -f." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-FILE-NOT_FOUND" "Not Found" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
remove_file()
{
    ...
    if [ -f "$1" ]; then
        ...
        write_error "REMOVE-FILE-NOT_FOUND" ": remove_file [$1] @ $2 -> $FUNCNAME @ $(basename $BASH_SOURCE) : $LINENO"
        print_error "REMOVE-FILE-NOT_FOUND" ": remove_file [$1] @ $2 -> $FUNCNAME @ $(basename $BASH_SOURCE) : $LINENO"
        return 1
    fi
} 
# -------------------------------------
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "REMOVE-FILES-USAGE"      "remove_files 1->(/Full-Path/) 2->(ext or Blank for *) 3->(Debugging Information)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-FILES-DESC"       "Remove Files if it exist." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-FILES-NOTES"      "if -f > rm -f." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-FILES-NOT_FOUND"  "Not Found" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-FILES-FNF"        "File Not Found Failed to remove files" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-FILES-FAIL"       "Failed to remove files" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
remove_files()
{
...
if [[ "$RUN_TEST" -eq 1 ]]; then
    ...
    remove_files "${FULL_SCRIPT_PATH}/Test/Target/Source/Extras/" "log" "remove_files @ $(basename $BASH_SOURCE) : $LINENO"
    if [ ! -f "${FULL_SCRIPT_PATH}/Test/Target/Source/Extras/*.log" ]; then
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "REMOVE-FOLDER-USAGE"     "remove_folder 1->(/Full-Path/) 2->(Debugging Information)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-FOLDER-DESC"      "Remove Folder if it exist." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-FOLDER-NOTES"     "if -d > rm -rf -- /Full-Path/" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-FOLDER-NOT_FOUND" "Not Found" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
remove_folder()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "DELETE-LINE-IN-FILE-USAGE" "delete_line_in_file 1->(Text to Delete) 2->(/FullPath/FileName.ext)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DELETE-LINE-IN-FILE-DESC"  "Given text of Line, Delete it in File" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DELETE-LINE-IN-FILE-NOTES" "This function is not ready to use, I forgot to finish it, so its just a stub that needs to be finished" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
delete_line_in_file()
{
...
# Help file Localization
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "COMMENT-FILE-USAGE" "comment_file 1->(Text to Comment) 2->(/FullPath/FileName.ext)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "COMMENT-FILE-DESC"  "Given text of Line, Comment it out in File" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "COMMENT-FILE-FNF"  "File not found" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
comment_file()
{
...
# Help file Localization
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "UN-COMMENT-FILE-USAGE" "un_comment_file 1->(Text) 2->(/FullPath/FileName.ext)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "UN-COMMENT-FILE-DESC"  "Given text of Line, un-Comment it out in File" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
un_comment_file()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "ADD-OPTION-USAGE" "add_option 1->(/Config/FullPath/FileName.ext) 2->(Option-String) 3->(Text to Add) 4->(Package Name)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-OPTION-DESC"  "Add Option: Given File Name, Option and Text, add option to end of line in file." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-OPTION-NOTES" "If you have a string in a file: i.e. '[Option]=', you can append Text to add to it: i.e. '[Option]=Text Added'" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-OPTION-SF"    "Warning: String Exist:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-OPTION-ERROR" "Error: String not Found:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-OPTION-FNF"   "File Not Found:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
add_option()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "REPLACE-OPTION-USAGE" "replace_option 1->(/Config/FullPath/FileName.ext) 2->(Option-String) 3->(Text to Replace) 4->(Package Name)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REPLACE-OPTION-DESC"  "Replace Option: Given File Name, Option and Text, add option to end of line in file." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REPLACE-OPTION-NOTES" "If you have a string in a file: i.e. '[Option]=', you can append Text to add to it: i.e. '[Option]=Text Added'" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REPLACE-OPTION-SF"    "Warning: String Exist:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REPLACE-OPTION-ERROR" "Error: String not Found:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REPLACE-OPTION-FNF"   "File Not Found:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
replace_option()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "MAKE-FILE-DESC"  "Make file." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "MAKE-FILE-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "MAKE-FILE-PASSED" "Make File created" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "MAKE-FILE-FAILED" "Failed to create File" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
make_file()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "SAVE-ARRAY-USAGE" "save_array 1->(Array(@)) 2->(/Path) 3->(MenuName.ext)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SAVE-ARRAY-DESC"  "Save Array." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SAVE-ARRAY-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SAVE-ARRAY-ERROR" "Error Saving Array." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
save_array()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "LOAD-ARRAY-USAGE" "Array=( &#36;(load_array 1->(/Path/ArrayName.ext) 2->(ArrarySize) 3->(Default Data) ) )" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "LOAD-ARRAY-DESC"  "Load a saved Array from Disk." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "LOAD-ARRAY-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
load_array()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "CREATE-DATA-ARRAY-USAGE" "create_data_array 1->(ArrarySize) 2->(Default Data)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-DATA-ARRAY-DESC"  "Create Data Array." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-DATA-ARRAY-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
create_data_array()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "IS-NUMBER-USAGE" "is_number 1->(value)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-NUMBER-DESC"  "Is Number." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-NUMBER-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
is_number()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "PRINT-MENU-USAGE" "print_menu 1->(MenuArray[@]) 2->(MenuInfoArray[@]) 3->(Letter to Exit)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PRINT-MENU-DESC"  "Print Menu." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PRINT-MENU-NOTES" "Localized." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "MENU-Q" "Quit" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "MENU-B" "Back" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "MENU-D" "Done" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
print_menu()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "ADD-MENU-ITEM-USAGE" "add_menu_item 1->(Checkbox_List_Array) 2->(Menu_Array) 3->(Info_Array) 4->(Menu Description in White) 5->(In Yellow) 6->(In Red) 7->(Information Printed Above Menu) 8->(MenuTheme_Array[@])" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-MENU-ITEM-DESC"  "Add Menu Item." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-MENU-ITEM-NOTES" "Text should be Localize ID." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
add_menu_item()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "IS-INTERNET-DESC"  "Check if Internet is up by Pinging two Major DNS servers." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-INTERNET-NOTES" "This pings google.com and wikipedia.org; they are good to ping to see if the Internet is up." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "IS-INTERNET-INFO" "Checking for Internet Connection..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
is_internet()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "IS-ONLINE-DESC"  "Check if URL can be Pinged through the Internet." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-ONLINE-NOTES" "This pings URL passed in." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "IS-ONLINE-INFO" "Checking URL for Internet Connection..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
is_online()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "ARRAY-PUSH-USAGE" "array_push 1->(array) 2->(Element)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ARRAY-PUSH-DESC"  "Push Element into an Array." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ARRAY-PUSH-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
array_push()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "REMOVE-FROM-ARRAY-USAGE" "remove_from_array 1->(array) 2->(Element)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-FROM-ARRAY-DESC"  "Remove Element from an Array." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-FROM-ARRAY-NOTES" "Pass in Array by name 'array'." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "REMOVE-FROM-ARRAY-ERROR" "Wrong Parameters passed to remove_from_array" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
remove_from_array()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "GET-INDEX-DESC"  "Get Index into an Array." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-INDEX-NOTES" "Bombs if not found; but finds errors in data; you could ask for data; but if its not in Array; this is a bug in Data not logic." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_index() 
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "REMOVE-ARRAY-DUPLICATES-USAGE" "MyArray=( \$( remove_array_duplicates '1->(Array[@])' ) )" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-ARRAY-DUPLICATES-DESC"  "Removes Duplicates in Array." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-ARRAY-DUPLICATES-NOTES" "set IFS=\$'\n\t'; before call" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "IS-LAST-ITEM-DESC"  "is last item in array." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-LAST-ITEM-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
is_last_item() 
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "DATE2STAMP-USAGE" "date2stamp 1->(date)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DATE2STAMP-DESC"  "Convert Date to Datetime stamp." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DATE2STAMP-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
date2stamp()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "DETECTED-VIDEO-CARD-DESC"  "Detect Video Card." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DETECTED-VIDEO-CARD-NOTES" "Need to add more support for Video Cards, currently it supports: nVidia, Intel, ATI and Vesa." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
detected_video_card()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "CLEAR-LOGS-DESC"    "Clear all Log Entries." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CLEAR-LOGS-NOTES"   "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CLEAR-LOGS-CLEAR-1" "Clearing Log Files" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CLEAR-LOGS-CLEAR-2" "Created Log Folders" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
clear_logs()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "IS-USER-DESC"  "Checks if USERNAME exist." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-USER-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
is_user()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "ADD-USER-GROUP-DESC"  "Add User group." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-USER-GROUP-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
add_user_group()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "REMOVE-USER-GROUP-USAGE" "remove_user_group 1->(Group Name)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-USER-GROUP-DESC"  "Remove User group." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-USER-GROUP-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
remove_user_group()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "IS-USER-IN-GROUP-DESC"  "is user in group." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-USER-IN-GROUP-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
is_user_in_group()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "IS-GROUP-DESC"  "Is Group." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-GROUP-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
is_group()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "ADD-GROUP-DESC"  "Add Group." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-GROUP-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-GROUP-OK"    "Added Group" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-GROUP-FAIL"  "Failed to add Group" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
add_group()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "ADD-USER-2-GROUP-DESC"  "Add User to Group." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-USER-2-GROUP-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "ADD-USER-2-GROUP-ERROR" "Error in adding User to group" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
add_user_2_group()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "SET-DEBUGGING-MODE-USAGE" "set_debugging_mode 1->(1=Boot, 2=Live) 2->(Debugging Information)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SET-DEBUGGING-MODE-DESC"  "Set Debugging Mode: also checks for Internet Connection." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SET-DEBUGGING-MODE-NOTES" "Fill try to Repair Internet Connection. Only sets Debugging switch if DEBUGGING is set to 1." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "SET-DEBUGGING-MODE-TITLE"         "Starting setup..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SET-DEBUGGING-MODE-INTERNET-UP"   "Internet is Up!" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SET-DEBUGGING-MODE-TRIED-TO-FIX"  "I tried to fix Network, I will test it again, if it fails, first try to re-run this script over, if that fails, try Network Troubleshooting." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SET-DEBUGGING-MODE-TRY-AGAIN"     "trying again in 13 seconds..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SET-DEBUGGING-MODE-INTERNET-DOWN" "Internet is Down: Internet is Down, this script requires an Internet Connection, fix and retry; try Network Troubleshooting; first try to rerun this script, I did try to fix this. Select Install with No Internet Connection option." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SET-DEBUGGING-MODE-NO-INTERNET"   "No Internet Install Set; if it fails; you must establish an Internet connection first; try Network Troubleshooting." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SET-DEBUGGING-MODE-WARN-1"        "Debug Mode will insert a Pause Function at critical functions and give you some information about how the script is running, it also may set other variables and run more test." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SET-DEBUGGING-MODE-WARN-2"        "Debugging is set on, if set -o nounset or set -u, you may get unbound errors that need to be fixed." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "BOOT-MODE-DETECTED"               "Boot Mode Detected." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "LIVE-MODE-DETECTED"               "Live Mode Detected." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
set_debugging_mode()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "GET-COUNTRY-CODES-DESC"  "country list." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-NOTES" "Sets COUNTRY." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-COUNTRY-CODES-SELECT" "Select your Country:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
country_list()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "GET-COUNTRY-CODES-DESC"    "Get Country Code and set Counter." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-NOTES"   "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-COUNTRY-CODES-WARN"    "You must enter your Country correctly, no validation is done!" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-INPUT"   "Country Code for Mirror List: (US) " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-TITLE"   "Country Code for Mirror List" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-COUNTRY-CODES-INFO-1"  "Australia     = AU | Belarus       = BY | Belgium       = BE" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-INFO-2"  "Brazil        = BR | Bulgaria      = BG | Canada        = CA" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-INFO-3"  "Chile         = CL | China         = CN | Colombia      = CO" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-INFO-4"  "Czech Repub   = CZ | Denmark       = DK | Estonia       = EE" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-INFO-5"  "Finland       = FI | France        = FR | Germany       = DE" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-INFO-6"  "Greece        = GR | Hungary       = HU | India         = IN" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-INFO-7"  "Ireland       = IE | Israel        = IL | Italy         = IT" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-INFO-8"  "Japan         = JP | Kazakhstan    = KZ | Korea         = KR" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-INFO-9"  "Macedonia     = MK | Netherlands   = NL | New Caledonia = NC" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-INFO-10" "New Zealand   = NZ | Norway        = NO | Poland        = PL" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-INFO-11" "Portugal      = PT | Romania       = RO | Russian Fed   = RU" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-INFO-12" "Serbia        = RS | Singapore     = SG | Slovakia      = SK" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-INFO-13" "South Africa  = ZA | Spain         = ES | Sri Lanka     = LK" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-INFO-14" "Sweden        = SE | Switzerland   = CH | Taiwan        = TW" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-INFO-15" "Ukraine       = UA | United King   = GB | United States = US" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODES-INFO-16" "Uzbekistan    = UZ | Viet Nam = VN" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_country_codes() 
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "GET-COUNTRY-CODE-DESC"    "Get Country and Country Code." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-COUNTRY-CODE-NOTES"   "Localized." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-COUNTRY-CODE-CONFIRM" "Confirm Country Code" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_country_code() 
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "GET-ROOT-PASSWORD-DESC"   "Get root password." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-PASSWORD-NOTES"  "This shows the password on screen; not very secure, but its used so you can see the password, you do not want a mistake putting in passwords." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-ROOT-PASSWORD-TITLE"  "root" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-PASSWORD-INFO-1" "No Special Characters, until I figure out how to do this." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-PASSWORD-INFO-2" "Enter Root Password." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-PASSWORD-VD"     "root Password" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-PASSWORD-INFO-3" "Root Password is Set." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_root_password()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "GET-USER-NAME-DESC"   "Get User Name." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-USER-NAME-NOTES"  "Sets USERNAME." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-USER-NAME-TITLE"  "User" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-USER-NAME-INFO-1" "No Special Characters, until I figure out how to do this." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-USER-NAME-INFO-2" "Enter User Name." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-USER-NAME-VD"     "User Name" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_user_name()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "GET-USER-PASSWORD-DESC"   "get user password." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-USER-PASSWORD-NOTES"  "Password in clear text." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-USER-PASSWORD-TITLE"  "User" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-USER-PASSWORD-INFO-1" "No Special Characters, until I figure out how to do this." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-USER-PASSWORD-INFO-2" "Enter User Password for" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-USER-PASSWORD-VD"     "User Password" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-USER-PASSWORD-INFO-3" "User Name and Password is Set." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_user_password()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "GET-LOCALE-DESC"     "Get Locale." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-LOCALE-NOTES"    "Used to get a Locale." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-LOCALE-TITLE"    "LOCALE" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-LOCALE-INFO-1"   "Locales are used in Linux to define which language the user uses." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-LOCALE-INFO-2"   "As the locales define the character sets being used as well, setting up the correct locale is especially important if the language contains non-ASCII characters." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-LOCALE-INFO-3"   "We can only initialize those Locales that are Available, if not in list, Install Language and rerun script." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-LOCALE-SELECT"   "Select your Language Locale:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-LOCALE-TITLE-2"  "LANGUAGE/LOCALE" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-LOCALE-INFO-4"   "Locales are used in Linux to define which language the user uses." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-LOCALE-INFO-5"   "As the locales define the character sets being used as well, setting up the correct locale is especially important if the language contains non-ASCII characters." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-LOCALE-INFO-6"   "We can only initialize those Locales that are Available, if not in list, Install Language and rerun script." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-LOCALE-INFO-7"   "First list shows all Available Languages, if yours is not in list choose No, then a full list will appear." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-LOCALE-INFO-8"   "Pick your Primary Language first, then you have an option to select as many languages as you wish." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-LOCALE-CONFIRM"  "Confirm Language Locale" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-LOCALE-DEFAULT"  "Use Default System Language"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-LOCALE-ADD-MORE" "Add more Locales" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-LOCALE-EDIT"     "Edit system language (ex: en_US): " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_locale()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "YES-NO-DESC"  "Convert Digital to Analog." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "YES-NO-NOTES" "Localized. Used to Show simple settings." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "YES" "Yes" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NO"  "No"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
yes_no()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "SELECT-CREATE-USER-DESC"  "select user." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SELECT-CREATE-USER-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "SUDO-WARNING" "WARNING: THE SELECTED USER MUST HAVE SUDO PRIVILEGES" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "Create-new-user" "Create new user" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SELECT-CREATE-USER-AVAILABLE-USERS" "Available Users: " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
select_create_user()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "RESTART-INTERNET-DESC"  "Restart Internet." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "RESTART-INTERNET-NOTES" "Assumes system.d; uses systemctl restart; needs generic functions for other then Arch Linux calls." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
restart_internet()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "FIX-NETWORK-DESC"  "Fix Network." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FIX-NETWORK-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "FIX-NETWORK-NETWORKMANAGER" "Restarting networkmanager via systemctl..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FIX-NETWORK-WICD"           "Restarting wicd via systemctl..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FIX-NETWORK-TRIED-TO-FIX"   "Tried to fix network connection; you may have to run this script again." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
fix_network()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "NETWORK-TROUBLESHOOTING-DESC"    "Network Troubleshooting." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-NOTES"   "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "NETWORK-TROUBLESHOOTING-TITLE"   "Network Troubleshooting" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-INFO-1"  "Network Debugging" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-INFO-2"  "Networkmanager: install and start, this is always the best way to start troubleshooting." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-INFO-3"  "Disk Resolv: Edit/Review namerservers.txt on disk, then copy it to local disk." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-INFO-4"  "Local Resolv:Edit/Review local /etc/resolv.conf" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-INFO-5"  "Identify which network interfaces" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-INFO-6"  "Link status: " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-INFO-7"  "IP Address: " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-INFO-8"  "Ping: " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-INFO-9"  "Devices: Show all ethx that are active" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-INFO-10" "Show Users: " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-INFO-11" "Static IP: " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-INFO-12" "Gateway: " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-INFO-13" "Quit" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-INFO-14" "Identify" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-INFO-15" "Link status" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-INFO-16" "Network Debugging" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-RD-1"    "Enter IP address (192.168.1.2) " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-RD-2"    "Enter IP Mask (255.255.255.0 = 24) " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-RD-3"    "Enter IP address for Gateway (192.168.1.1) " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-SELECT"  "Select an Option:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NETWORK-TROUBLESHOOTING-NIC"     "Select a NIC:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "GET-KEYBOARD-LAYOUT-DESC"   "Get Keyboard Layout, makes changes for some variants." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-KEYBOARD-LAYOUT-NOTES"  "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-KEYBOARD-LAYOUT-TITLE"  "Keymap." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-KEYBOARD-LAYOUT-SELECT" "Select keyboard layout:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_keyboard_layout()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "CONFIGURE-KEYMAP-DESC"    "Allows user to decide if they wish to change the Default Keymap." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-KEYMAP-NOTES"   "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "Load-Keymap"              "Load Keymap" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "Confirm-Keymap"           "Confirm Keymap" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-KEYMAP-TITLE"   "KEYMAP" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-KEYMAP-INFO"    "The KEYMAP variable is specified in the /etc/rc.conf file. It defines what keymap the keyboard is in the virtual consoles. Keytable files are provided by the kbd package." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-KEYMAP-DEFAULT" "If Default is ok, then no changes needed: " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-KEYMAP-LAYOUT"  "Keyboard Layout (ex: us-acentos): " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "CONFIGURE-KEYMAP-INFO-1"  "Belgian                = be-latin1    | Brazilian Portuguese = br-abnt2     | Canadian-French = cf" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-KEYMAP-INFO-2"  "Canadian Multilingual  = ca_multi     | Colemak (US)         = colemak      | Croatian        = croat" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-KEYMAP-INFO-3"  "Czech                  = cz-lat2      | Dvorak               = dvorak       | French          = fr-latin1" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-KEYMAP-INFO-4"  "German                 = de-latin1 or de-latin1-nodeadkeys                  | Italian         = it" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-KEYMAP-INFO-5"  "Lithuanian             = lt.baltic    | Norwegian            = no-latin1    | Polish          = pl" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-KEYMAP-INFO-6"  "Portuguese             = pt-latin9    | Romanian             = ro_win       | Russian         = ru4" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-KEYMAP-INFO-7"  "Singapore              = sg-latin1    | Slovene              = slovene      | Swedish         = sv-latin1" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-KEYMAP-INFO-8"  "Swiss-French           = fr_CH-latin1 | Swiss-German         = de_CH-latin1 | Spanish         = es" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-KEYMAP-INFO-9"  "Spanish Latinoamerican = la-latin1    | Turkish              = tr_q-latin5  | Ukrainian       = ua" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-KEYMAP-INFO-10" "United States          = us or us-acentos                                   | United Kingdom  = uk" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
configure_keymap()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "GET-EDITOR-DESC"  "This gets called from Boot mode and Live mode; it does not add software, only ask if you wish to change the default editor, called from the create_config function." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-EDITOR-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-EDITOR-DEFAULT"   "Do you wish to change the Default Editor of " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-EDITOR-TITLE"     "DEFAULT EDITOR" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-EDITOR-INSTALLED" "Installed Editor(s): " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-EDITOR-EDITORS"   "Editors" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-EDITOR-SELECT"    "Select default editor:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_editor()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "SELECT-EDITOR-DESC"  "This gets called from Boot mode only; it installs on the Boot OS, and schedules an install on the Live OS." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SELECT-EDITOR-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
select_editor()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "CONFIGURE-TIMEZONE-DESC"     "Configure Timezone." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-NOTES"    "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "CONFIGURE-TIMEZONE-DEFAULT"  "Is the Default Timezone Correct" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-CONFIRM"  "Confirm Timezone " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-TITLE"    "TIMEZONE" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-INFO-1"   "In an operating system the time (clock) is determined by four parts: Time value, Time standard, Time Zone, and DST (Daylight Saving Time if applicable)." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-ZONE"     "Select zone:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-SUBZONE"  "Select subzone:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "CONFIGURE-TIMEZONE-ZONE-1"   "Africa" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-ZONE-2"   "America" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-ZONE-3"   "Antarctica" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-ZONE-4"   "Arctic" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-ZONE-5"   "Asia" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-ZONE-6"   "Atlantic" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-ZONE-7"   "Australia" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-ZONE-8"   "Brazil" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-ZONE-9"   "Canada" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-ZONE-10"  "Chile" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-ZONE-11"  "Europe" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-ZONE-12"  "Indian" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-ZONE-13"  "Mexico" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-ZONE-14"  "Midest" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-ZONE-15"  "Pacific" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-TIMEZONE-ZONE-16"  "US" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
configure_timezone()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    ...
    # -----------------------------------------------------------------------------
    # create_help
    localize_info "CREATE-HELP-USAGE"   "create_help 1->(NAME of Function.) 2->(USAGE) 3->(DESCRIPTION) 4->(NOTES) 5->(AUTHOR) 6->(VERSION) 7->(CREATED) 8->(REVISION) 9->(Source File and LINENO)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-HELP-DESC"    "Create an HTML Help File on the Fly" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-HELP-NOTES"   "This Allows easy reading and Look up of all Functions in Program.<br />${HELP_TAB}This Function must be first Function all scripts see, so put it at the top of file.<br />${HELP_TAB}You can get as elaborate with help files as you want." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-HELP-WORKING" "Create Help Working" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PRINT-HELP-ERROR"    "Help Array Empty!" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    # -----------------------------------------------------------------------------
    # print_help
    localize_info "PRINT-HELP-DESC"  "Print an HTML Help File on the Fly" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PRINT-HELP-NOTES" "This Allows easy reading and Look up of all Functions in Program." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PRINT-HELP-TITLE" "Arch Wizard Help File Generated on" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PRINT-HELP-FUNCT" "Function" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    # -----------------------------------------------------------------------------
    # string_len
    localize_info "STRING-LEN-USAGE"   "string_len 1->('String')" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "STRING-LEN-DESC"    "Given String, returns Length." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "STRING-LEN-NOTES"   "Calls C function." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    # -----------------------------------------------------------------------------
    # sub_string
    localize_info "SUB-STRING-USAGE"   "sub_string 1->('String') 2->(Search) 3->(1=Beginning, 2=End, 3=Remove) <- returns in var MyString" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SUB-STRING-DESC"    "Returns Sub Strings" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SUB-STRING-NOTES"   "See Run Test examples." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    # -----------------------------------------------------------------------------
    # PAUSE FUNCTION {{{
    localize_info "PAUSE-FUNCTION-USAGE"   "pause_function 1->(Description Debugging Information)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PAUSE-FUNCTION-DESC"    "Pause function" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PAUSE-FUNCTION-NOTES"   "Localized: Arguments passed in are not Localize, this is used for passing in Function names, that can not be localized; if required: localize before passing in." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "PRESS-ANY-KEY-CONTINUE" "Press any key to continue" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    # ----------------------------------------------------------------------------- 
    # WRITE ERROR
    localize_info "WRITE-ERROR-USAGE" "write_error 1->(Error) 2->(Debugging Information)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "WRITE-ERROR-DESC"  "Write Error to log." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "WRITE-ERROR-NOTES" "Localized." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "WRITE-ERROR-ARG"   "Wrong Number of Arguments passed to write_error!" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "NOT-FOUND"         "Not Found" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    # -----------------------------------------------------------------------------
    # Help file Localization
    localize_info "TRIM-DESC"   "Remove space on Right and Left of string" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "TRIM-NOTES"  "MY_SPACE=' Left and Right '<br />${HELP_TAB}MY_SPACE=&#36;(trim &#34;&#36;MY_SPACE&#34;)<br />${HELP_TAB}echo &#34;|&#36;(trim &#34;&#36;MY_SPACE&#34;)|&#34;" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "LTRIM-NOTES" "MY_SPACE=' Left and Right '<br />${HELP_TAB}MY_SPACE=&#36;(ltrim &#34;&#36;MY_SPACE&#34;)<br />${HELP_TAB}echo &#34;|&#36;(ltrim &#34;&#36;MY_SPACE&#34;)|&#34;" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "RTRIM-NOTES" "MY_SPACE=' Left and Right '<br />${HELP_TAB}MY_SPACE=&#36;(rtrim &#34;&#36;MY_SPACE&#34;)<br />${HELP_TAB}echo &#34;|&#36;(rtrim &#34;&#36;MY_SPACE&#34;)|&#34;" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    # -----------------------------------------------------------------------------
    # Help file Localization
    localize_info "LEFT-TRIM-DESC"  "Remove space on Left of string" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    # -----------------------------------------------------------------------------
    # Help file Localization
    localize_info "RIGHT-TRIM-USAGE" "rtrim 1->(' String to Trim ')" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "RIGHT-TRIM-DESC"  "Remove space on Right of string" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    # -----------------------------------------------------------------------------
    # Help file Localization
    localize_info "IS-IN-ARRAY-USAGE" "is_in_array 1->(Array{@}) 2->(Search)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-IN-ARRAY-DESC"  "Is Search in Array{@}; return true (0) if found" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-IN-ARRAY-NOTES" "Use of Global ARR_INDEX can be used in array index: if is_in_array 'Array{@}' 'Search' ; then MyArray{ARR_INDEX}=1 ; fi; much like get_index; which bombs on not found; takes more code to write it." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    # -----------------------------------------------------------------------------
    # Help file Localization 
    localize_info "IS-NEEDLE-IN-HAYSTACK-USAGE" "is_needle_in_haystack 1->(Needle to search in Haystack) 2->(Haystack to search in) 3->(Type of Search: 1=Exact, 2=Beginning, 3=End, 4=Middle, 5=Anywhere)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-NEEDLE-HAYSTACK-DESC"  "Search for a Needle in the Haystack" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-NEEDLE-HAYSTACK-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    # -----------------------------------------------------------------------------
    # Help file Localization
    localize_info "LOAD-2D-ARRAY-DESC"    "Load a saved 2D Array from Disk" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "LOAD-2D-ARRAY-NOTES"   "This Function Expects a file, bombs if not found." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "LOAD-2D-ARRAY-MISSING" "Missing File" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    # -----------------------------------------------------------------------------
    # localize_save
    ...
    # -----------------------------------------------------------------------------
    # Help file Localization
    localize_info "LOCALIZE-INFO-DESC"  "Localize Info creates the &#36;{FULL_SCRIPT_PATH}/Localize/en.po file used for Localization." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "LOCALIZE-INFO-NOTES" "Localized." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "LOCALIZE-INFO-USAGE" "localize_info 1->(Localize ID) 2->(Message to Localize) 3->(Comment)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-LOCALIZER-WORKING" "Create Localizer Working..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "WRONG-NUMBER-ARGUMENTS-PASSED-TO" "Wrong Number of Arguments passed to " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "IS-STRING-IN-FILE-USAGE" "is_string_in_file 1->(/full-path/file) 2->(search for string)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-STRING-IN-FILE-DESC"  "Return true if string is in file." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-STRING-IN-FILE-NOTES" "Used to test files for Updates." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "IS-STRING-IN-FILE-FNF" "File Not Found" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    # -----------------------------------------------------------------------------
    # Show Help
    localize_info "SHOW-HELP-INFO-USAGE-1"  "Usage:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-USAGE-2"  "./ScriptName &nbsp;&nbsp;   # Run Script in Interactive Mode." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-USAGE-3"  "./ScriptName -l # Run Localizer" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-USAGE-4"  "./ScriptName -h # Build Help File, must run -l first." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-USAGE-5"  "./ScriptName -a # Automatically Run Scripts without Prompts." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-USAGE-6"  "./ScriptName -t # Run Test in mode 1" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-USAGE-7"  "./ScriptName -s # Run Special Test in mode 2" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-USAGE-8"  "./ScriptName -x # Run eXtra Test in mode 3" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    # -----------------------------------------------------------------------------
    # About
    localize_info "SHOW-HELP-INFO-1"  "The Wizard API script was designed to help in writing complex scripts, it handles must of the input and handles common functions." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-2"  "Wizard API:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-3"  "&nbsp;&nbsp;&nbsp;&nbsp; The Wizard API is the base of the script engine used to write this script, which in itself only writes another script," "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-4"  "&nbsp;&nbsp;&nbsp;&nbsp; so this is known as a script engine, whereas the API or Application Programming Interface, is the syntax," "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-5"  "&nbsp;&nbsp;&nbsp;&nbsp; which is the parameters sent to the function, as such Documenting all the functions would be a huge undertaking in most projects this size," "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-6"  "&nbsp;&nbsp;&nbsp;&nbsp; so I decided to make this script self Documenting, as well as self Localizing, a non-localized script is worthless to the world," "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-7"  "&nbsp;&nbsp;&nbsp;&nbsp; in a perfect Society we would all talk the same Language, for me that would be C++, " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-8"  "&nbsp;&nbsp;&nbsp;&nbsp; so lets just say that no one can agree on what Language to speak in, let alone program in, so even this text needs to be translated," "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-9"  "&nbsp;&nbsp;&nbsp;&nbsp; for those that do not read English; and this is static text; so these instructions really need to be in the script itself;" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-10" "&nbsp;&nbsp;&nbsp;&nbsp; which is why its self Documenting; so it can translate that into the language the person reading it can read it in," "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-11" "&nbsp;&nbsp;&nbsp;&nbsp; so that is it for this static file, all other Documentation will be built in." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-12" "&nbsp;&nbsp;&nbsp;&nbsp; Every program ever write should do 3 things, besides running flawlessly:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-13" "&nbsp;&nbsp;&nbsp;&nbsp; 1. Localized for every language that will be using it." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-14" "&nbsp;&nbsp;&nbsp;&nbsp; 2. Self Documenting." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-15" "&nbsp;&nbsp;&nbsp;&nbsp; 3. Self Testing, ability to run Test and Determine if program is working correctly." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-16" "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-17" "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-18" "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-HELP-INFO-19" "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    # -----------------------------------------------------------------------------
    # Troubleshooting Functions
    localize_info "TEST-FUNCTION-PASSED"      "Test Passed" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "TEST-FUNCTION-FAILED"      "Test Failed" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "TEST-FUNCTION-RUN"         "Running Test" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "TEST-FUNCTION-FNF"         "File Not Found" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "WRONG-NUMBER-OF-ARGUMENTS" "Wrong Number of Arguments" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "INSTALL-MENU-RECOMMENDED"  "Recommended Menu Options" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "INSTALL-MENU-COMPLETED"    "Completed." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -----------------------------------------------------------------------------
show_help()
{
...
# -------------------------------------
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "READ-INPUT-OPTIONS-TEST-1"  "Test Options." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-TEST-2"  "Option :" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-TEST-3"  "Testing Menu System with Options: " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-TEST-4"  "Status of Option :" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-TEST-5"  "Menu System Test" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "READ-INPUT-OPTIONS-MENU-1"    "Menu 1" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-1-I"         "Menu 1: Information." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-1-C"         "Caution" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-1-W"         "Warning" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-2"    "Menu 2" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-2-I"         "Menu 2: Information." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-2-C"         "Caution" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-2-W"         "Warning" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-3"    "Menu 3" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-3-I"         "Menu 3: Information." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-3-C"         "Caution" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-3-W"         "Warning" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-4"    "Menu 4" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-4-I"         "Menu 4: Information." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-4-C"         "Caution" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-4-W"         "Warning" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-5"    "Menu 5" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-5-I"         "Menu 5: Information." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-5-C"         "Caution" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-5-W"         "Warning" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-6"    "Menu 6" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-6-I"         "Menu 6: Information." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-6-C"         "Caution" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-6-W"         "Warning" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-7"    "Menu 7" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-7-I"         "Menu 7: Information." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-7-C"         "Caution" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-7-W"         "Warning" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-8"    "Menu 8" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-8-I"         "Menu 8: Information." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-8-C"         "Caution" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-8-W"         "Warning" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-9"    "Menu 9" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-9-I"         "Menu 9: Information." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-9-C"         "Caution" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-9-W"         "Warning" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-10"   "Menu 10" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-10-I"        "Menu 10: Information." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-10-C"        "Caution" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "READ-INPUT-OPTIONS-MENU-10-W"        "Warning" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
if [[ "$RUN_TEST" -eq 2 || "$RUN_TEST" -eq 2 || "$RUN_TEST" -eq 3 ]]; then
    test_read_input_options()
    {
        ...
        while [[ 1 ]]; do
            ...
            #
            add_menu_item "MenuChecks" "MenuItems" "MenuInfo" "READ-INPUT-OPTIONS-MENU-1"  "READ-INPUT-OPTIONS-MENU-1-C"  "READ-INPUT-OPTIONS-MENU-1-W"  "READ-INPUT-OPTIONS-MENU-1-I"  "MenuTheme[@]"
            add_menu_item "MenuChecks" "MenuItems" "MenuInfo" "READ-INPUT-OPTIONS-MENU-2"  "READ-INPUT-OPTIONS-MENU-2-C"  "READ-INPUT-OPTIONS-MENU-2-W"  "READ-INPUT-OPTIONS-MENU-2-I"  "MenuTheme[@]"
            add_menu_item "MenuChecks" "MenuItems" "MenuInfo" "READ-INPUT-OPTIONS-MENU-3"  "READ-INPUT-OPTIONS-MENU-3-C"  "READ-INPUT-OPTIONS-MENU-3-W"  "READ-INPUT-OPTIONS-MENU-3-I"  "MenuTheme[@]"
            add_menu_item "MenuChecks" "MenuItems" "MenuInfo" "READ-INPUT-OPTIONS-MENU-4"  "READ-INPUT-OPTIONS-MENU-4-C"  "READ-INPUT-OPTIONS-MENU-4-W"  "READ-INPUT-OPTIONS-MENU-4-I"  "MenuTheme[@]"
            add_menu_item "MenuChecks" "MenuItems" "MenuInfo" "READ-INPUT-OPTIONS-MENU-5"  "READ-INPUT-OPTIONS-MENU-5-C"  "READ-INPUT-OPTIONS-MENU-5-W"  "READ-INPUT-OPTIONS-MENU-5-I"  "MenuTheme[@]"
            add_menu_item "MenuChecks" "MenuItems" "MenuInfo" "READ-INPUT-OPTIONS-MENU-6"  "READ-INPUT-OPTIONS-MENU-6-C"  "READ-INPUT-OPTIONS-MENU-6-W"  "READ-INPUT-OPTIONS-MENU-6-I"  "MenuTheme[@]"
            add_menu_item "MenuChecks" "MenuItems" "MenuInfo" "READ-INPUT-OPTIONS-MENU-7"  "READ-INPUT-OPTIONS-MENU-7-C"  "READ-INPUT-OPTIONS-MENU-7-W"  "READ-INPUT-OPTIONS-MENU-7-I"  "MenuTheme[@]"
            add_menu_item "MenuChecks" "MenuItems" "MenuInfo" "READ-INPUT-OPTIONS-MENU-8"  "READ-INPUT-OPTIONS-MENU-8-C"  "READ-INPUT-OPTIONS-MENU-8-W"  "READ-INPUT-OPTIONS-MENU-8-I"  "MenuTheme[@]"
            add_menu_item "MenuChecks" "MenuItems" "MenuInfo" "READ-INPUT-OPTIONS-MENU-9"  "READ-INPUT-OPTIONS-MENU-9-C"  "READ-INPUT-OPTIONS-MENU-9-W"  "READ-INPUT-OPTIONS-MENU-9-I"  "MenuTheme[@]"
            add_menu_item "MenuChecks" "MenuItems" "MenuInfo" "READ-INPUT-OPTIONS-MENU-10" "READ-INPUT-OPTIONS-MENU-10-C" "READ-INPUT-OPTIONS-MENU-10-W" "READ-INPUT-OPTIONS-MENU-10-I" "MenuTheme[@]"
            #
