...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "REFRESH-PACMAN-DESC"  "Refresh pacman" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "REFRESH-PACMAN-INFO"  "Refresh Pacman Database..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
refresh_pacman()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "SYSTEM-UPDATE-DESC"  "Full pacman System Upgrade: Set a var so you do not do this every call, then perform a pacman-optimize" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "SYSTEM-UPDATE-INFO"  "UPDATING YOUR Pacman SYSTEM" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
system_upgrade()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "UPDATE-SYSTEM-DESC"  "Update system" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "UPDATE-SYSTEM-INFO"  "Update system" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
update_system()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "INSTALLED-CORE-USAGE" "installed_core 1->(package-name)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "INSTALLED-CORE-DESC"  "Installed core" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
installed_core()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "FAILED-INSTALL-CORE-USAGE" "failed-install_core 1->(package-name)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FAILED-INSTALL-CORE-DESC"  "Failed install core" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
failed_install_core()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "INSTALLED-AUR-USAGE" "installed_aur 1->(package-name)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "INSTALLED-AUR-DESC"  "Installed aur" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
installed_aur()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "FAILED-INSTALL-AUR-USAGE" "failed_install_aur 1->(package-name)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FAILED-INSTALL-AUR-DESC"  "Failed install aur" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
failed_install_aur()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "CHECK-PACKAGE-USAGE" "check_package 1->(Single-Package-to-Check Or Multiple Packages)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CHECK-PACKAGE-DESC"  "checks package(s) to see if they are installed in pacman." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CHECK-PACKAGE-NOTES" "I have seen this fail for one or more packages that were already install: mate; so I added -Qm for this reason." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "CHECK-PACKAGE-NFCD" "Not Found in pacman Database" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CHECK-PACKAGE-FSSP" "Found in pacman -Ssp" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CHECK-PACKAGE-FFP"  "Failed to find package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
check_package()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "SET-LANGUAGE-DESC"  "Set Language: Used to set Languages for packages" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
set_language()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "DOWNLOAD-AUR-REPO-PACKAGES-DESC"  "Download AUR Repository Packages: download all AUR Repository Packages into one folder, unarchive them, compile them, and copy package to Custom Package Repo" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "DOWNLOAD-AUR-REPO-PACKAGES-NO-PACKAGES"          "No packages in download_aur_repo_packages; create Software Configuration first." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DOWNLOAD-AUR-REPO-PACKAGES-DOWNLOADING"          "Downloading AUR Repository with" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DOWNLOAD-AUR-REPO-PACKAGES-DOWNLOADING-PACKAGE"  "AUR downloading package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DOWNLOAD-AUR-REPO-PACKAGES-PACKAGE-FAIL"         "aur_download_packages Failed to download package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DOWNLOAD-AUR-REPO-PACKAGES-COMPLETED"            "Download AUR Repo Packages Completed" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "COPY-CUSTOM-REPO-USAGE" "copy_custom_repo 1->(/From-Full-Path) 2->(Repo Name) 3->(/To-Full-Path/)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "COPY-CUSTOM-REPO-DESC"  "Copy Custom Repository" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "COPY-CUSTOM-REPO-INFO"  "Copying Custom Repository..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
copy_custom_repo()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "DOWNLOAD-REPO-PACKAGES-DESC"          "Download Repository Packages: download all Arch/Repository Packages into one folder, unarchive them, compile them, and copy package to Custom Package Repo." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "DOWNLOAD-REPO-PACKAGES-NO-PACKAGES"   "No Packages: create Software Configuration first: " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DOWNLOAD-REPO-PACKAGES-DOWNLOADING"   "pacman Downloading Package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DOWNLOAD-REPO-PACKAGES-CREATING"      "Creating Custom Repo with" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DOWNLOAD-REPO-PACKAGES-DOWNLOAD-FAIL" "pacman Failed to Download Package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DOWNLOAD-REPO-PACKAGES-NOT-FOUND"     "Not Found Files" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DOWNLOAD-REPO-PACKAGES-REPO-ADD-FAIL" "repo-add failed for" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DOWNLOAD-REPO-PACKAGES-COMPLETED"     "Download Repo Packages Completed" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CREATE-CUSTOM-AUR-REPO-USAGE" "create_custom_aur_repo 1->( 1=archwiz Install OS, 2=packagewiz Install Software)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-CUSTOM-AUR-REPO-DESC"  "Create a Custom Repository; so you can do installs real fast after the first try." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-CUSTOM-AUR-REPO-NOTES" "If we have more than one Arch Box; we want a common Repository; mounted drive or NAS; plus we want to copy those files to local cache." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "CREATE-CUSTOM-AUR-REPO-ERROR"  "Failed Download AUR Repo Packages for" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
create_custom_aur_repo()
{
    # In Boot Mode; hard drive is formatted and pacstrap has not completed.
    if [[ "$RUNTIME_MODE" -eq 1 ]]; then
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CREATE-CUSTOM-REPO-USAGE" "create_custom_repo 1->( 1=archwiz Install OS, 2=packagewiz Install Software)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-CUSTOM-REPO-DESC"  "Create a Custom Repository; so you can do installs real fast after the first try." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-CUSTOM-REPO-NOTES" "If we have more than one Arch Box; we want a common Repository; mounted drive or NAS; plus we want to copy those files to local cache <br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;/var/lib/pacman/sync/{core,extra,testing,community}<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;https://wiki.archlinux.org/index.php/Pacman_Tips#Network_shared_pacman_cache<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;https://wiki.archlinux.org/index.php/Custom_local_repository_with_ABS_and_gensync#Custom_local_repository<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;This Function assumes you have configured database, and will download all software into a Repository, if one is chosen; otherwise it just copies them to cache." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "CREATE-CUSTOM-REPO-ERROR-DOWNLOAD-REPO"          "Failed Download Repo Packages for" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-CUSTOM-REPO-COPYING-REPO-LOCAL-CACHE"     "Coping Repository to Local Cache..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-CUSTOM-REPO-COPYING-REPO-SHARE-CACHE"     "Coping Repository to Sharable Cache..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-CUSTOM-REPO-CLIENT-REPO"                  "Setting up Client Repository..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-CUSTOM-REPO-NO-REPO"                      "No Repository." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-CUSTOM-REPO-EDIT-PACMAN-CONF-REPO-CACHE"  "Edit pacman.conf for Repository cache..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-CUSTOM-REPO-COMPLETE"                     "Create Custom Repo Completed" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
create_custom_repo()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "ADD-REPO-USAGE" "add_repo 1->(repo-name) 2->(url) 3->(trust-level) 4->(1=add &#36;arch to url)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-REPO-DESC"  "Add repo: Ran from Task Manager." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-REPO-NOTES" "trust-level: Optional TrustAll, PackageRequired, Never" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
add_repo()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "REQUIRED-REPO-DESC"  "Add Required Repository." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REQUIRED-REPO-NOTES" "Ran from Boot Mode in Task Manager." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
required_repo()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "OPTIMIZE-PACMAN-DESC"  "Optimize Pacman to use parallel downloads using Aria2, rsync and ppl parisync" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "OPTIMIZE-PACMAN-NOTES" "Implement: UseDelta install xdelta3" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "OPTIMIZE-PACMAN-FAILED"    "PACMAN Optimization Failed." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "OPTIMIZE-PACMAN-COMPLETED" "PACMAN Optimization Completed." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
optimize_pacman()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "INSTALL-PACKAGE-USAGE" "install_package 1->(NAME_OF_PACKAGE) 2->(NAME-OF-PACKAGE-MANAGER)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "INSTALL-PACKAGE-DESC"  "Install package from core or additional Repositories." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "INSTALL-PACKAGE-NOTES" "Install one at a time, check to see if already install, if fails, try again with with confirm." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "INSTALL-PACKAGE-INSTALLING"     "pacman Installing" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "INSTALL-PACKAGE-ERROR-1"        "pacman did not install Package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "INSTALL-PACKAGE-ERROR-2"        "for Install Package Manager" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "INSTALL-PACKAGE-INSTALL-PM2ML"  "using pm2ml: Installing" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
install_package()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "PACKAGE-INSTALL-USAGE" "package_install 1->(SPACE DELIMITED LIST OF PACKAGES TO INSTALL) 2->(NAME-OF-PACKAGE-MANAGER)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACKAGE-INSTALL-DESC"  "Install package from core or additional Repositories." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACKAGE-INSTALL-NOTES" "Install one at a time, check to see if already install, if fails, try again with with confirm." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "PACKAGE-INSTALL-ERROR"            "pacman did not install Package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACKAGE-INSTALL-FAILED-INTERNET"  "Internet check failed" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACKAGE-INSTALL-PACKAGE-MANAGER"  "for Package Manager" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACKAGE-INSTALL-REFRESH"          "Refreshing pacman Database and Updates..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACKAGE-INSTALL-COMPLETE"         "Package Manager Completed" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACKAGE-INSTALL-FAILURES"         "Installed" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACKAGE-INSTALL-PACKAGES"         "Packages" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACKAGE-INSTALL-FAILED"           "Failed" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACKAGE-INSTALL-INSTALLED"        "Installed" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACKAGE-INSTALL-RETRY"            "Retry" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACKAGE-INSTALL-PACKAGE"          "install package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACKAGE-INSTALL-MANUAL"           "with Manual intervention." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACKAGE-INSTALL-CURRENTLY"        "currently working on" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACKAGE-INSTALL-RETRIES"          "Retries" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
package_install()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "PACKAGE-REMOVE-USAGE" "package_remove 1->(SPACE DELIMITED LIST OF PACKAGES TO REMOVE) 2->(NAME-OF-PACKAGE-MANAGER)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACKAGE-REMOVE-DESC"  "Package remove" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "PACKAGE-REMOVE-INFO"  "Removing package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
package_remove()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "DO-CURL-USAGE"         "do_curl 1->(name of AUR Package)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DO-CURL-DESC"          "Download AUR Package file via curl." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "DO-CURL-DOWNLOADED"    "Downloaded AUR Package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DO-CURL-FAILED"        "Failed to Downloaded AUR Package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DO-CURL-PASSED"        "Downloaded AUR Package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DO-CURL-DOWNLOADING"   "Downloading:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DO-CURL-ONLINE-FAILED" "Failed to ping https://aur.archlinux.org" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
do_curl()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "AUR-DOWNLOAD-DESC"  "AUR Download Packages" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-DOWNLOAD-NOTES" "&#36;AUR_CUSTOM_PACKAGES: if Boot Mode /root/usb/AUR-Packages, if Live Mode /mnt/AUR-Packages" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "AUR-DOWNLOAD-DOWNLOADING-AUR-PACKAGE" "Downloading AUR Package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-DOWNLOAD-FILE-EXIST"              "File Exist, check date of file" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-DOWNLOAD-PACKAGE-UP2DATE"         "Up to date Package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-DOWNLOAD-FILE-CORRUPTED"          "File Corrupted:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-DOWNLOAD-FILE-NOT-FOUND"          "File Not Found" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-DOWNLOAD-CURL-FAILED"             "Failed to Downloaded AUR Package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
aur_download()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-AUR-PACKAGES-DESC"             "Get AUR packages" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-AUR-PACKAGES-NOTES"            "Called from export under user, so script functions not available, cd folder before calling" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-AUR-PACKAGES-COMPILING"        "Compiling Package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-AUR-PACKAGES-FAILED-COMPILING" "Failed to compile" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_aur_packages()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "AUR-DOWNLOAD-PACKAGES-DESC"  "AUR Download Packages" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-DOWNLOAD-PACKAGES-NOTES" "AUR_CUSTOM_PACKAGES: if Boot Mode /root/usb/AUR-Packages, if Live Mode /mnt/AUR-Packages" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "AUR-DOWNLOAD-PACKAGES-TITLE"              "AUR Package Downloader." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-DOWNLOAD-PACKAGES-WARN-CREATE-FOLDER" "Could not create folder " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-DOWNLOAD-PACKAGES-WORKING-ON"         "Working on Package " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-DOWNLOAD-PACKAGES-RETRIES"            "retries" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-DOWNLOAD-PACKAGES-FAILED-DOWNLOAD"    "Failed Downloading AUR Package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "INSTALL-DOWNLOAD-USAGE" "install_download 1->(PACKAGE FROM AUR) 2->(Args: NoConfirm, Force) 3->(Log output)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "INSTALL-DOWNLOAD-DESC"  "Install AUR Package using AUR_HELPER" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "INSTALL-DOWNLOAD-NOTES" "Called from add_packagemanager, run in Live Mode: Install one at a time, check to see if its already installed, if fail, try again with confirm.<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;: --needed --recursive --force --upgrades" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
install_download()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "AUR-PACKAGE-INSTALL-USAGE" "aur_package_install 1->(SPACE DELIMITED LIST OF PACKAGES TO INSTALL FROM AUR) 2->(NAME-OF-PACKAGE-MANAGER)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-PACKAGE-INSTALL-DESC"  "Install AUR Package using AUR_HELPER" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-PACKAGE-INSTALL-NOTES" "Called from add_packagemanager, run in Live Mode: Install one at a time, check to see if its already installed, if fail, try again with confirm." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "AUR-PACKAGE-INSTALL-WORKING-ON"     "AUR Package Install" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-PACKAGE-INSTALL-TOTAL-INSTALL"  "Total and number for installed is not equal" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-PACKAGE-INSTALL-CURRENTLY"      "currently Working on" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-PACKAGE-INSTALL-OF"             "of" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-PACKAGE-INSTALL-PACKAGES"       "packages" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-PACKAGE-INSTALL-INSTALLED"      "Installed" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-PACKAGE-INSTALL-FAIL"           "Fails" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-PACKAGE-INSTALL-RETRIES"        "retries" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-PACKAGE-INSTALL-REFRESH"        "Refreshing" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-PACKAGE-INSTALL-ERROR-1"        "Error: Did not install AUR Package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-PACKAGE-INSTALL-ERROR-2"        "Package Manager" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-PACKAGE-INSTALL-INTERNET"       "Internet check failed." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-PACKAGE-INSTALL-UPDATE"         "Updating" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-PACKAGE-INSTALL-NOT-INSTALLED"  "Package(s) Not Installed" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-PACKAGE-INSTALL-COMPLETE"       "AUR Package Manager Completed" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "AUR-PACKAGE-INSTALL-TRY-AGAIN"      "Try install again"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
aur_package_install()
{
...
if [[ "$RUN_HELP" -eq 1 ]]; then
    ...
    DESCRIPTION=$(localize "PACMAN-PACKAGE-SIGNING-DESC")
    NOTES=$(localize "NONE")
    AUTHOR="helmuthdu and Flesher"
    VERSION="1.0"
    CREATED="11 Sep 2012"
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "PACMAN-PACKAGE-SIGNING-DESC"      "Configure pacman package signing." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "PACMAN-PACKAGE-SIGNING-TITLE"     "PACMAN PACKAGE SIGNING" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACMAN-PACKAGE-SIGNING-INFO"      "Pacman-key is a new tool available with pacman 4. It allows the user to manage pacmans list of trusted keys in the new package signing implementation." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "PACMAN-PACKAGE-SIGNING-COMPLETE"  "Pacman Package Signing Configured" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
configure_pacman_package_signing()
{
    if [ ! -d /etc/pacman.d/gnupg ]; then
        print_title "PACMAN-PACKAGE-SIGNING-TITLE" " - https://wiki.archlinux.org/index.php/Pacman-key"
        print_info  "PACMAN-PACKAGE-SIGNING-INFO"
    ...
    print_this "PACMAN-PACKAGE-SIGNING-COMPLETE"
}
#}}}
# -----------------------------------------------------------------------------
# CREATE CONFIG SKEL {{{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CREATE-CONFIG-SKEL-USAGE" "create_config_skel 1->(config name) 2->(/Full/Path/) 3->(FileName.ext) 4->(extra)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-CONFIG-SKEL-DESC"  "Create a Configuration File Skeleton." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CREATE-CONFIG-SKEL-NOTES" "None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
create_config_skel()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CONFIG-XINITRC-USAGE" "config_xinitrc 1->(module name)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIG-XINITRC-DESC"  "Edit .xinitrc file for manual start of Desktop" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIG-XINITRC-NOTES" "Info: https://wiki.archlinux.org/index.php/Xinitrc" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
config_xinitrc()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CHOOSE-AUR-HELPER-DESC"   "Choose AUR Helper" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "CHOOSE-AUR-HELPER-TITLE"   "AUR HELPER" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CHOOSE-AUR-HELPER-INFO-1"  "AUR Helpers are written to make using the Arch User Repository more comfortable." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CHOOSE-AUR-HELPER-INFO-2"  "YAOURT" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CHOOSE-AUR-HELPER-INFO-3"  "Yaourt (Yet AnOther User Repository Tool) is a community-contributed wrapper for pacman which adds seamless access to the AUR, allowing and automating package compilation and installation from your choice of the thousands of PKGBUILDs in the AUR, in addition to the many thousands of available Arch Linux binary packages." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CHOOSE-AUR-HELPER-INFO-4"  "Yaourt is Recommended." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CHOOSE-AUR-HELPER-INFO-5"  "List of AUR Helpers: yaourt, packer and pacaur." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CHOOSE-AUR-HELPER-INFO-6"  "None of these tools are officially supported by Arch devs." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CHOOSE-AUR-HELPER-SELECT"  "Choose your default AUR helper to install and use, you can only use one." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CHOOSE-AUR-HELPER-CHANGE"  "Do you wish to change the Default AUR Helper"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
choose_aurhelper()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "LOAD-CUSTOM-SOFTWARE-DESC"  "load custom software" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "LOAD-CUSTOM-SOFTWARE-NOTES" "@FIX write function" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "LOAD-CUSTOM-SOFTWARE-INFO"  "Edit or review file first." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-MIRRORLIST-USAGE" "get_mirrorlist 1->(Country Code)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-MIRRORLIST-DESC"  "get mirrorlist" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-MIRRORLIST-DOWNLOADING"  "Downloading pacman Mirror" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_mirrorlist() 
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CONFIGURE-MIRRORLIST-DESC"  "Configure mirrorlist" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "CONFIGURE-MIRRORLIST-FOUND"  "Mirrorlist found on" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-MIRRORLIST-MIRRORS"  "MIRRORS" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-MIRRORLIST-INFO"  "Edit or Review your mirrorlist and exit to continue." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "Use-Mirrorlist" "Do you wish to use"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "READ-NAMESERVERS-DESC"  "Read Nameserver" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "READ-NAMESERVERS-ERROR"  "Missing" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
read_nameserver()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CUSTOM-NAMESERVERS-DESC"       "Custom Nameservers" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "CUSTOM-NAMESERVERS-CORRECT"    "Is this correct" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-NAMESERVERS-USE"        "Use custom nameserers"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-NAMESERVERS-INFO-1"     "The resolver is a set of routines in the C library that provide access to the Internet Domain Name System (DNS)." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-NAMESERVERS-INFO-2"     "The resolver configuration file contains information that is read by the resolver routines the first time they are invoked by a process." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-NAMESERVERS-INFO-3"     "The file is designed to be human readable and contains a list of keywords with values that provide various types of resolver information." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-NAMESERVERS-INFO-4"     "On a normally configured system this file should not be necessary." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-NAMESERVERS-INFO-5"     "The only name server to be queried will be on the local machine; the domain name is determined from the host name and the domain search path is constructed from the domain name." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-NAMESERVERS-INFO-6"     "OpenDNS servers 208.67.222.222 and 208.67.220.220" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-NAMESERVERS-INFO-7"     "Google nameservers 8.8.8.8 and 8.8.4.4" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-NAMESERVERS-INFO-8"     "Or use the ones form your Local ISP, or own DNS Servers." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-NAMESERVERS-WARN-1"     "You must enter two nameservers correctly, no validation is done!" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-NAMESERVERS-DEFAULT-1"  "Enter NAMESERVER 1 (123.123.123.123): " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-NAMESERVERS-DEFAULT-2"  "Enter NAMESERVER 2 (123.123.123.123): " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-NAMESERVERS-DEFAULT-3"  "Enter Search (ex: (sub-domain.)url.tdl or Blank (enter) for none): " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
custom_nameservers()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-FLESH-DESC"     "Set FLESH, so Custom User Settings can be Created, add some Flesh to this Bare Bones Skeleton" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-FLESH-TITLE"    "Pacman Tips" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FLESH-INFO"     "The Basic Bare Bones Skeleton install is good, but its best to add some Flesh to it, by Customizing the user folder with some extra Configuration from helmuthdu, and Flesher." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FLESH-INSTALL"  "Install Flesh" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_flesh()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CUSTOM-REPOSITORIES-DESC"  "Add Custom Repositories" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "CUSTOM-REPOSITORIES-ADD-REPO"     "Add Custom Repositories" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-REPOSITORIES-TITLE"        "Custom Repositories" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-REPOSITORIES-INFO"         "Do not add &#36;arch to the end, the script will do that for you." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-REPOSITORIES-NEW"          "Add new Repository" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-REPOSITORIES-REPO-NAME"    "Repository Name (ex: custom): " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-REPOSITORIES-REPO-ADDRESS" "Repository Address (ex: http://domain.url.tdl/archlinux): " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-REPOSITORIES-TRUST-LEVEL"  "Select your Trust Level:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CUSTOM-REPOSITORIES-APPEND-ARCH"  "Append arch to end of url for auto detect Architecture: 64 or 32 bit." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "BACKUP-FILES-DESC"  "Backup files" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
backup_files()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CREATE-CONFIG-DESC"  "Create Configuration files for Last Config." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
create_config()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CHANGE-USER-DESC"  "Change user in Configuration files." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
change_user()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "EDIT-DISK-DESC"  "Edit Disk Configuration files." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "EDIT-DISK-EDIT-DRIVE" "Edit Drive with gdisk"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "EDIT-DISK-IS-SSD-DISK" "Is this an SSD Disk"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "EDIT-DISK-INFO-1"  "After Drive is formatted, you can edit it using gdisk." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "EDIT-DISK-INFO-2"  "Special Settings can help with SSD Drives." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
edit_disk()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "SHOW-PACKMANAGER-DESC"  "Show All Packages to Manage" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
show_packmanager()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "SHOW-SOFTWARE-DESC"          "Show Loaded Variables for Software Configuration" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-NOTES"         "Configuration files exist" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "SHOW-SOFTWARE-WARN-USERS"    "No User Groups set." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "SHOW-SOFTWARE-INFO-1"        "Software Configuration Database is built using the Full install menu; then saved to this file; the menu settings are store also, so you can visually see what you installed." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-INFO-2"        "If the Menu's get out of sync with the Database, they will have to be deleted; the software handles this automatically; but if you know the Database is out of sync; create a new one." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-INFO-3"        "If you want to create a New Software Configuration file; you must not load the last one, or clear the Arrays and delete the files, this is handled automatically if you chose a new Configuration." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-LOAD-SOFTWARE" "Load Software Configuration Database"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-VIEW-PACKAGES" "View Package Manager Commands: "  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SCROLL-INFO"   "Use Shift-PageUp and Down if scrolls off screen" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-NEW-CONFIG"    "Do you wish to Create a New Software Configuration File" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-INFO-ERROR"    "Software Config failed to load" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-INFO-VAR"      "Configuration and Software Install Variables:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-INFO-4"        "The Menus shows you what you installed last time; the configuration will be overwritten; as such, it does not save what you have, unless you loaded it earlier; which is the case to get here." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-INFO-5"        "You have two Options; Create a New Software Configuration File, or edit this one; which relies on the Menu System and Menu Database being in sync." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "SHOW-SOFTWARE-SETTING-1"     "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-2"     "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-3"     "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-4"     "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-5"     "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-6"     "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-7"     "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-8"     "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-9"     "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-10"    "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-11"    "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-12"    "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-13"    "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-14"    "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-15"    "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-16"    "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-17"    "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-18"    "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-19"    "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-SOFTWARE-SETTING-20"    "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
show_software()
{
    ...
    if [[ "YN_OPTION" -eq 1 ]]; then
        ...
        echo -e "POSTGRESQL           = ${BWhite}$POSTGRESQL${White}"
        ...
        echo -e "QT INSTALL           = ${BWhite}$(yes_no $QT_INSTALL)         ${White}"
        echo -e "RAZOR QT INSTALLED   = ${BWhite}$(yes_no $RAZOR_QT_INSTALLED) ${White}"
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "SHOW-LOADED-DESC"               "Show Loaded Variables" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-LOADED-NOTES"              "Localized." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "SHOW-LOADED-LAST-CONFIG"        "Last Configuration Database contain User specific Settings." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-LOADED-LOAD-USER-CONFIG"   "Load User Configuration Database" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-LOADED-FAILED"             "Last Config Failed to load at line" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-LOADED-BELOW"              "The below Configuration Settings can be changed without re-running Software install." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-LOADED-USERNAME"           "You can just change User Name (Y), or whole configuration (N)." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-LOADED-CHANGE-USERNAME"    "Do you wish to change User Name" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-LOADED-EDIT"               "Do you wish to edit these settings" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
show_last_config()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "SHOW-DISK-PROFILE-DESC"  "Show Disk Profile" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "SHOW-DISK-PROFILE-TITLE"  "Disk Profile: Configuration Variables:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
show_disk_profile()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "DETECT-MODE-DESC"  "Detect Mode: Detects Boot mode: Boot Mode from ISO, Live Preinstall, and Live OS Mode after Installation of Software has ran." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
detect_mode()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-INSTALL-MODE-DESC"  "Change MOUNTPOINT" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-INSTALL-MODE"                          "Install mode has 3 Options: Boot and Live Preinstall and Live OS Mode:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-INSTALL-MODE-BOOT"                     "Boot mode is when you Boot from an Arch ISO to install OS for first time, not to be confused with LIVE CD." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-INSTALL-MODE-LIVE"                     "Live Preinstall mode is after you install Boot, and then reboot into Live Preinstall mode, i.e. Active Installed OS, after you install OS, then Reboot, you will then be in Live OS Mode." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-INSTALL-MODE-IS-BOOT"                  "Is Install Mode Boot" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-INSTALL-MODE-IS-LIVE"                  "Is Install Mode Live OS" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-INSTALL-MODE-IS-LIVE-PREINSTALL"       "Is Install Mode Live Preinstall" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-INSTALL-MODE-BOOT-DETECTED"            "Boot Mode Detected." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-INSTALL-MODE-LIVE-PREINSTALL-DETECTED" "Live Preinstall Mode Detected." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-INSTALL-MODE-LIVE-DETECTED"            "Live Mode Detected." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-INSTALL-MODE-MODES"                    "1. Boot 2. Live Preinstall 3. Live OS" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-INSTALL-MODE-WHICH"                    "Which Mode are you in" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_install_mode()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "VERIFY-CONFIG-USAGE" " 1->(1=Install OS, 2=Install Software)"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-CONFIG-DESC"  "Verify Configuration settings and load all files or create new ones." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "VERIFY-CONFIG-OPTIMIZATION"          "Do you wish to Configure pacman Optimization"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-CONFIG-CREATE-NEW-CONFIG"     "Do you wish to Create a New Software Configuration File"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-CONFIG-DISK-PROFILE"          "Do you wish change Disk Profile"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-CONFIG-KEEP-DISK-PROFILE"     "Do you wish to continue with this Disk Profile"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-CONFIG-COPY-FOLDER"           "Do you wish to Copy saved Flash Folder: /etc/ to /"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-CONFIG-PACMAN-OPTIMIZATION"   "pacman Optimization will configure system to use aria2 reflector and pm2ml; it will also configure a local repository which can be share across a local Network; this will help download files faster." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-CONFIG-CHECKING-OPTIMIZE"      "Checking for pacman Optimization" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-CONFIG-NO-OPTIMIZE"           "No Optimized pacman!" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-CONFIG-NO-CONFIG"             "No Configuration file found." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-CONFIG-SOFTWARE-CONFIG-FOUND" "A Software Configuration file has not been detected, you can Create a New one." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-CONFIG-VERIFY-DISK-SETTINGS"  "You now need to verify the Disk Settings" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-CONFIG-RUN-SCRIPT-AGAIN"      "Run Script again, after you decide on a Disk Profile." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-CONFIG-FOLDER-ETC-HOLDS"      "The Folder etc holds various configurations files like: pacman.conf, fstab, hostname, hosts, resolve.conf, sudoers, timezone, vconsole.conf, and mirrorlist." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "VERIFY-CONFIG-CONFIG-COMPLETED"      "Configuration File Check Completed..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
verify_config()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CLEAR-SOFTWARE-DESC"      "Clear All variables that effect Software" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "CLEAR-SOFTWARE-ERROR"     "No Menus found" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CLEAR-SOFTWARE-INFO"      "Removed Menus" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CLEAR-SOFTWARE-COMPLETE"  "Removed Software Configuration" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
clear_software()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "SAVE-INSTALL-DESC"  "Save Install list to LOG_PATH." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
save_install()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "SAVE-SOFTWARE-DESC"  "Save Software Configuration: All variables that effect PACKMANAGER" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
save_software()
{
    ...
    echo "$WEBSERVER"          >> "${CONFIG_PATH}/${CONFIG_NAME}-4-software-config.db"
    # POSTGRESQL
    echo "?POSTGRESQL"         >> "${CONFIG_PATH}/${CONFIG_NAME}-4-software-config.db"
    echo "$POSTGRESQL"         >> "${CONFIG_PATH}/${CONFIG_NAME}-4-software-config.db"
    # CONFIG_ORPHAN
    ...
    echo "$KDE_INSTALLED"      >> "${CONFIG_PATH}/${CONFIG_NAME}-4-software-config.db"
    # QT_INSTALL
    echo "?QT_INSTALL"         >> "${CONFIG_PATH}/${CONFIG_NAME}-4-software-config.db"
    echo "$QT_INSTALL"         >> "${CONFIG_PATH}/${CONFIG_NAME}-4-software-config.db"
    # RAZOR_QT_INSTALLED
    echo "?RAZOR_QT_INSTALLED" >> "${CONFIG_PATH}/${CONFIG_NAME}-4-software-config.db"
    echo "$RAZOR_QT_INSTALLED" >> "${CONFIG_PATH}/${CONFIG_NAME}-4-software-config.db"
    # LXDE_INSTALLED
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "SAVE-LAST-CONFIG-DESC"  "Save Last Configuration file." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
save_last_config()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "SAVE-DISK-CONFIG-DESC"  "Save Disk Configuration" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
save_disk_config()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "IS-SOFTWARE-FILES-DESC"    "Test if all Config files exist" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-SOFTWARE-FILES-MISSING" "File Missing! Will create new one." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
is_software_files()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "LOAD-SOFTWARE-DESC"  "Load Software Configuration files" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "LOAD-SOFTWARE-ERROR" "No Configuration File" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
load_software()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "IS-LAST-CONFIG-FILES-DESC"  "Test if all Config files exist" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-LAST-CONFIG-FILES-FNF"  "File Missing! Will create new one." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "LOAD-LAST-CONFIG-DESC" "Last Configuration is User Specific Settings like USERNAME, LOCALE, HOSTNAME, KEYMAP COUNTRY, TIMEZONE and more." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "LOAD-LAST-CONFIG-FNF"  "File Not Found" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
load_last_config()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "LOAD-DISK-CONFIG-DESC"  "Load Disk Configuration" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
load_disk_config()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "ADD-PACKAGEMANAGER-USAGE" "add_packagemanager 1->(COMMAND-LINE) 2->(NAME-OF-PACKAGE)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-PACKAGEMANAGER-DESC"  "Add A Package to the Manager" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-PACKAGEMANAGER-NOTES" "Hart of System." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "ADD-PACKAGEMANAGER-ERROR"  "Wrong Parameters passed to add_packagemanager" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
add_packagemanager()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "REMOVE-PACKAGEMANAGER-USAGE" "remove_packagemanager 1->(NAME-OF-PACKAGE)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-PACKAGEMANAGER-DESC"  "Remove A Package to the Manager" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-PACKAGEMANAGER-NOTES" "Hart of System." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "REMOVE-PACKAGEMANAGER-ERROR"  "Wrong Parameters to add_packagemanager" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "ADD-TASKMANAGER-USAGE" "add_taskmanager 1->(COMMAND-LINE) 2->(NAME-OF-PACKAGE)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-TASKMANAGER-DESC"  "Add A Task to the Manager" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "ADD-TASKMANAGER-ERROR"  "Wrong Parameters to add_taskmanager" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
add_taskmanager()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "ADD-PACKAGE-USAGE" "add_package 1->(package)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-PACKAGE-DESC"  "Add Package to PACKAGES array; for testing and building cache folder" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-PACKAGE-NOTES" "Call per Package Manager" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "ADD-PACKAGE-ERROR"  "Wrong Parameters to add_package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
add_package()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "REMOVE-PACKAGE-USAGE" "remove_package 1->(package)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-PACKAGE-DESC"  "Remove Package from PACKAGES array; for testing and building cache folder" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-PACKAGE-NOTES" "Call per Package Manager" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "REMOVE-PACKAGE-ERROR"  "Wrong Parameters to remove_package" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
remove_package()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "ADD-AUR-PACKAGE-USAGE" "add_aur_package 1->(package)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-AUR-PACKAGE-DESC"  "Add AUR Package to PACKAGES array; for testing and building cache folder." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-AUR-PACKAGE-NOTES" "Call per AUR Package Manager" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "ADD-AUR-PACKAGE-ERROR"  "Wrong Parameters to" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
add_aur_package()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "REMOVE-AUR-PACKAGE-USAGE" "remove_aur_package 1->(package)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-AUR-PACKAGE-DESC"  "Remove AUR Package from PACKAGES array." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-AUR-PACKAGE-NOTES" "Call per AUR Package Manager" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "REMOVE-AUR-PACKAGE-ERROR"  "Wrong Parameters to" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
remove_aur_package()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "ADD-MODULE-USAGE" "add_module 1->(Name of Module) 2->(Package Manager)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-MODULE-DESC"  "Add Module to Package Manager." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ADD-MODULE-NOTES" "Call per Module" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
add_module()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "REMOVE-MODULE-USAGE" "remove_module 1->(Package Manager)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-MODULE-DESC"  "Remove Module from Package Manager." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "REMOVE-MODULE-NOTES" "Call per Module" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
remove_module()
{ 
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "USER-CONFIG-USAGE" "user_config 1->(USERNAME)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "USER-CONFIG-DESC"  "User Configuration settings." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "USER-CONFIG-NOTES" "This is run by User, Library is unavailable, unless you export all the functions to use it." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "USER-CONFIG-TITLE"      "Configuring User:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "USER-CONFIG-COMPLETED"  "User Configuration Completed." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "USER-CONFIG-ERROR-1"    "Failure in creating .Xauthority; try generate new one." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "USER-CONFIG-ERROR-2"    "Failure in creating .Xauthority; try add new one." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "USER-CONFIG-ERROR-3"    "Failure in creating .Xauthority, giving up and creating file." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "USER-CONFIG-ERROR-4"    "User Configuration Completed with Errors." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
user_config()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CONFIGURE-USER-ACCOUNT-DESC"  "This function gets called after software is installed, so configure what is needed here." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "CONFIGURE-USER-ACCOUNT-TITLE"     "Configuring User Account" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-USER-ACCOUNT-CONFIG"    "Configuring :" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-USER-ACCOUNT-INFO"      "BASHRC" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-USER-ACCOUNT-ERROR-1"   "configure_user_account: vim failed to Download" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-USER-ACCOUNT-ERROR-2"   "configure_user_account: pentadactyl failed to Download" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-USER-ACCOUNT-ERROR-3"   "configure_user_account: dotfiles failed to Download" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-USER-ACCOUNT-ERROR-4"   "configure_user_account: Failure in creating .Xauthority for user" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-USER-ACCOUNT-ERROR-5"   "configure_user_account: Failure in creating .xinitrc for user" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-USER-ACCOUNT-COMPLETED" "Configuring User Account Completed." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-USER-ACCOUNT-CONFIG"    "Configuring Dot Files..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
configure_user_account()
{
    ...
    if [[ "$INSTALL_NFS" -eq 1 ]]; then
        echo ''
        replace_option "/etc/idmapd.conf" "Domain =" "$CONFIG_HOSTNAME"
    fi
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-AUR-PACKAGE-FOLDER-DESC"  "Get AUR package folder and AUR Repository Setting." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-AUR-PACKAGE-FOLDER-NOTES" "Notes." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-AUR-PACKAGE-USE"           "Use AUR Repository" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-AUR-PACKAGE-ENTER-NAME"    "Enter Name for AUR Packages" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-AUR-PACKAGE-FOLDER-INFO-1" "This is the Folder where AUR Projects get Downloaded into and Compiled At." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    ...
    localize_info "GET-AUR-PACKAGE-FOLDER-INFO-3" "Even if you do not use the Repository, you still have to download and compile AUR projects here." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    ...
    localize_info "GET-AUR-PACKAGE-FOLDER-INFO-5" "AUR Repository allows you to share your AUR Packages in your Local Network; it can also make updates easier." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-FSTAB-CONFIG-DESC"  "Get fstab config" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-FSTAB-CONFIG-TITLE"      "FSTAB" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FSTAB-CONFIG-INFO"       "The /etc/fstab file contains static filesystem information. It defines how storage devices and partitions are to be mounted and integrated into the overall system. It is read by the mount command to determine which options to use when mounting a specific device or partition." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FSTAB-CONFIG-RECOMMEND"  "UUID is the preferred way to create fstab." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FSTAB-CONFIG-CONFIGURE"  "Configure fstab based on:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FSTAB-CONFIG-EDIT"       "Edit fstab file"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_fstab_config()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CONFIGURE-FSTAB-DESC"  "Configure fstab" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-FSTAB-NOTES" "Localized." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "CONFIGURE-FSTAB-INFO" "Configuring fstab..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-FSTAB-EDIT" "The /etc/fstab file contains static filesystem information. It defines how storage devices and partitions are to be mounted and integrated into the overall system. It is read by the mount command to determine which options to use when mounting a specific device or partition." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-FSTAB-REVIEW" "Review your fstab." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CONFIGURE-FSTAB-COMPLETE" "fstab configuration complete." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
configure_fstab()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CONFIGURE-HOSTNAME-DESC"  "Configure hostname" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "CONFIGURE-HOSTNAME-COMPLETE"  "Configured Hostname..." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
configure_hostname()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-HOSTNAME-DESC"    "Get hostname" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-HOSTNAME-TITLE"   "HOSTNAME" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-HOSTNAME-INFO-1"  "A host name is a unique name created to identify a machine on a network.Host names are restricted to alphanumeric characters." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-HOSTNAME-INFO-2"  "The hyphen (-) can be used, but a host name cannot start or end with it. Length is restricted to 63 characters." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-HOSTNAME-INFO-3"  "Do not add any comments or empty lines." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-HOSTNAME-INFO-4"  "Do not use a domain name." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-HOSTNAME-DEFAULT" "Hostname (ex: archlinux)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_hostname()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "ABORT-INSTALL-USAGE" "abort_install 1->(1=Boot OS Install, 2=Install Software)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ABORT-INSTALL-DESC"  "Abort Install for Errors" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "ABORT-INSTALL-INFO-1"  "I made a back copy of the profile in /root; you can delete these files." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ABORT-INSTALL-ERROR"   "Drive is now ready to install Software, reboot, and remount Flash Drive with Script, cd /Path2Profile, load saved profile, or create a new one." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ABORT-INSTALL-INFO-2"  "You should now have a full system install, just reboot and you are ready to go." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
abort_install()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "FINISH-USAGE"     "finish 1->(1=Boot OS Install, 2=Install Software)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FINISH-DESC"      "Finish with script; clean up and do backups" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "FINISH-TITLE"     "Finish" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FINISH-INFO-1"    "INSTALL COMPLETED" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FINISH-INFO-2"    "If all went right you should be able to reboot into a fully functioning Desktop." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FINISH-INFO-3"    "Make sure to check the root for install files like install_scripts, install_scripts_root_secrets, install_scripts_user_secrets, you can also delete /boot/grub_uefi.log." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FINISH-INFO-4"    "I made a back copy of the profile in /root; you can delete these files." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FINISH-ERROR"     "Drive is now ready to install Software, reboot, and remount Flash Drive with Script, cd /Path2Profile, load saved profile, or create a new one." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FINISH-INFO-5"    "You should now have a full system install, just reboot and you are ready to go." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FINISH-REBOOT"    "Reboot system"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FINISH-SUDO-ERR1" "Error in sudorers, string not found."  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FINISH-SUDO-ERR2" "Error in sudorers, failed to remove comment."  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FINISH-REPO"      "Remove Custom Repository"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
finish()
{
    ...
    if [[ "$1" -eq 1 ]]; then # Boot Mode
        ...
        # @FIX add question/option to remove Custom Repo
        read_input_yn "FINISH-REPO" " " 1
        if [[ "$YN_OPTION" -eq 1 ]]; then
            if is_string_in_file "/etc/pacman.conf" "$FILE_SIGNATURE" ; then
            
                #comment_custom_repo=`grep -n "\[${CUSTOM_REPO_NAME}\]" "/etc/pacman.conf" | cut -f1 -d:`
                #if $comment_custom_repo &> /dev/null; then
                #    echo "NF"
                #else
                    #sed -i 's/^'[${CUSTOM_REPO_NAME}]'/#'[${CUSTOM_REPO_NAME}]'/g' "/etc/pacman.conf"
                     sed '/[${CUSTOM_REPO_NAME}]/,/Server = file://mnt/s/^/#/'  /etc/pacman.conf
                #    sed -i "${comment_custom_repo}s/^#//" "/etc/pacman.conf"
                #    comment_custom_repo=$(( $comment_custom_repo + 1 ))
                #    sed -i "${comment_custom_repo}s/^#//" "/etc/pacman.conf"
                #    comment_custom_repo=$(( $comment_custom_repo + 1 ))
                #    sed -i "${comment_custom_repo}s/^#//" "/etc/pacman.conf"
                #fi
                ...
                if ! comment_file "[custom]" "/etc/pacman.conf" ; then #    !authenticate
                    #sed -i "/# uncommented to enable the repo./a #\n${FILE_SIGNATURE}\n# Commented below lines to disable the Custom Repository.\n[${CUSTOM_REPO_NAME}]\nSigLevel = Optional TrustedOnly\nServer = file://${myMOUNTPOINT}/${CUSTOM_PACKAGES_NAME}/" "/etc/pacman.conf"
                    #sed -i '/\[options\]/a CleanMethod = KeepCurrent' "/etc/pacman.conf"
                    echo ''
                fi
    ...
    # I get input errors; like a stuffed key; @FIX find a clear key input function
    pause_function "$FUNCNAME @ $(basename $BASH_SOURCE) : $LINENO"
    #
    Old_BYPASS="$BYPASS"; BYPASS=0; # Do Not Allow Bypass
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "IS-UEFI-MODE-DESC"  "Is UEFI Mode will return True (0), if it is in UEFI Mode" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "IS-UEFI-MODE-INFO"  "UEFI Mode Test Passed." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "IS-UEFI-MODE-WARN"  "UEFI Mode Test Failed! UEFI Installation will not work correctly, switching to BIOS mode." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
is_uefi_mode()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "SET-LOG-DRIVE-DESC"  "Get Device that Script is running from." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SET-LOG-DRIVE-NOTES" "Localized." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "Correct-Device"        "Is this the Correct Device that this Script is running from"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "Device-Script"         "Device Script is running from, used to Copy Profile (Logs, Configuration and Database files) to Drive, this is normally the Flash Drive you use to run this Script from." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "Device-Script-running" "What Device is Script running from" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "Copying-Device"        "Copying to Device" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "Device-does-not-exist" "Device does not exist! Try running script again using correct device name, run fdisk -l for a list of Device names." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
set_log_drive()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "FIX-REPO-DESC"  "Fix Repository in pacman configure." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "FIX-REPO-INFO-1"  "Edit" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FIX-REPO-INFO-2"  "Put Mate into" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FIX-REPO-INFO-3"  "Copy" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FIX-REPO-INFO-4"  "Quit." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "FIX-REPO-SELECT"  "Select a Repository to fix :" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
fix_repo()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-FORMAT-TYPE-DESC"  "Get format type" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-FORMAT-TYPE-ERROR"  "format not recognized." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_format_type()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-FORMAT-SYSTEM-DESC"  "Get format system" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-FORMAT-SYSTEM-TITLE"    "Get File Format System: " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-1"   "ext2  Second Extended Filesystem is an established, mature GNU/Linux filesystem that is very stable." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-2"   "A drawback is that it does not have journaling support or barriers." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-3"   "Lack of journaling can result in data loss in the event of a power failure or system crash." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-4"   "It may also be inconvenient for root (/) and /home partitions because file-system checks can take a long time." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-5"   "An ext2 filesystem can be converted to ext3." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-6"   "ext3  Third Extended Filesystem is essentially the ext2 system with journaling support and write barriers." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-7"   "It is backward compatible with ext2, well tested, and extremely stable." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-8"   "ext4  Fourth Extended Filesystem is a newer filesystem that is also compatible with ext2 and ext3." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-9"   "It provides support for volumes with sizes up to 1 exabyte (i.e. 1,048,576 terabytes) and files sizes up to 16 terabytes." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-10"  "It increases the 32,000 subdirectory limit in ext3 to 64,000. It also offers online defragmentation capability." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-11"  "Btrfs Also known as 'Better FS', Btrfs is a new filesystem with powerful features similar to Sun/Oracle's excellent ZFS." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-12"  "These include snapshots, multi-disk striping and mirroring (software RAID without mdadm), checksums, incremental backup, and on-the-fly compression that can give a significant performance boost as well as save space." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-13"  "As of January 2011, Btrfs is considered unstable although it has been merged into the mainline kernel with an experimental status." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-14"  "Btrfs appears to be the future of GNU/Linux filesystems and is offered as a root filesystem option in all major distribution installers." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-15"  "vfat  or Virtual File Allocation Table is technically simple and supported by virtually all existing operating systems." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-16"  "This makes it a useful format for solid-state memory cards and a convenient way to share data between operating systems." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-17"  "VFAT supports long file names." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-INFO-18"  "NTFS  File system used by windows. Mountable with many utilities (e.g. NTFS-3G)." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-FORMAT-SYSTEM-SELECT"   "Select Format Type:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_format_system()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-BOOT-TYPE-DESC"  "Get boot type" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-BOOT-TYPE-TITLE"       "Get BIOS Size of GPT." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-1"      "GRUB2 in BIOS-GPT configuration requires a BIOS Boot Partition to embed its core.img in the absence of post-MBR gap in GPT partitioned systems (which is taken over by the GPT Primary Header and Primary Partition table)." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-2"      "This partition is used by GRUB2 only in BIOS-GPT setups." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-3"      "No such partition type exists in case of MBR partitioning (at least not for GRUB2)." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-4"      "This partition is also not required if the system is UEFI based, as no embedding of boot sectors takes place in that case." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-5"      "Syslinux does not require this partition." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-6"      "You can boot using BIOS or UEFI if you have a UEFI BIOS, setup depends on type of Boot System." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-SELECT"      "Select a Boot BIOS System Type:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-SPECIFY"     "Specify BIOS Partition size as such: 1M or 1G" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-DEFAULT"     "BIOS Recommend Size: (2M) " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-7"      "The UEFISYS partition can be of any size supported by FAT32 filesystem." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-8"      "According to Microsuck Documentation, the minimum partition/volume size for FAT32 is 512 MiB." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-9"      "Therefore it is recommended for UEFISYS partition to be at least 512 MiB." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-10"     "Higher partition sizes are fine, especially if you use multiple UEFI boot loaders, or multiple OSes booting via UEFI, so that there is enough space to hold all the related files." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-11"     "If you are using Linux EFISTUB booting, then you need to make sure there is adequate space available for keeping the Kernel and Initramfs files in the UEFISYS partition." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-12"     "Specify UEFI Partition size as such: 512M or 1G" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-UEFI-MIN"    "UEFI Minimal Size: (512M)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-SELECT-BIOS" "Select a Boot BIOS System Type:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-TITLE-2"     "UEFI Boot Loader" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-13"     "Unified Extensible Firmware Interface (or UEFI for short) is a new type of firmware that was initially designed by Intel (known as EFI then) mainly for its Itanium based systems." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-14"     "It introduces new ways of booting an OS that is distinct from the commonly used 'MBR boot code' method followed for BIOS systems." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-15"     "It started as Intel's EFI in versions 1.x and then a group of companies called the UEFI Forum took over its development from which it was called Unified EFI starting with version 2.0 ." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-16"     "As of 23 May 2012, UEFI Specification 2.3.1 is the most recent version." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-17"     "The NONE Option will not create a Boot Partition, it will install a Bootloader on the root partition." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-18"     "You have 3 Options for Boot Mode: UEFI (if you have a UEFI BIOS, Recommended), BIOS, or None (Installing to root, not recommended)." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "Default-Boot-Mode"          "Do you wish to change the Default Boot Mode: "  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-WARN-1" "Your Motherboard failed to register in UEFI Mode, so this script will only be able to use BIOS Mode or None." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-WARN-2" "You do not have a UEFI BIOS, switching to BIOS Mode." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-INFO-WARN-3" "You have an Option have no partition for Boot, this will force it to load to the Root, this is not recommended, but an option." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "DEFAULT-NONE"              "Default is BIOS mode, do you wish to change Boot to NONE"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-GRUB2"       "Grub2" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-SYSLINUX"    "Syslinux" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-TYPE-SKIP"        "Skip" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_boot_type()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-BOOT-PARTITION-DESC"          "Get boot partition" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-BOOT-PARTITION-TITLE"         "Get Boot Partition" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-PARTITION-INFO-1"        "The /boot directory contains the kernel and ramdisk images as well as the bootloader configuration file and bootloader stages." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-PARTITION-INFO-2"        "It also stores data that is used before the kernel begins executing user-space programs." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-PARTITION-INFO-3"        "/boot is not required for normal system operation, but only during boot and kernel upgrades (when regenerating the initial ramdisk)." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-PARTITION-INFO-4"        "If kept on a separate partition, /boot does not require a journaled file system." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-PARTITION-INFO-5"        "A separate /boot partition is needed if installing a software RAID0 (stripe) system." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-PARTITION-INFO-6"        "Do not confuse BOOT with UEFI or BIOS Boot strap, where as BOOT contains the Kernels, UEFI and BIOS contain Grub only." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-PARTITION-SEPARATE-PART" "Use boot on a separate Partition"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-PARTITION-SPECIFY"       "Specify BOOT Partition size as such: 100M or 1G" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-BOOT-PARTITION-BOOT-SIZE"     "BOOT Size: (100M)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_boot_partition()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-SWAP-PARTITION-DESC"           "Get swap partition" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-SWAP-PARTITION-TITLE"          "Get swap partition" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-SWAP-PARTITION-INFO-1"         "The swap partition provides memory that can be used as virtual RAM. It is recommended for PCs with 1GB or less of physical RAM." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-SWAP-PARTITION-INFO-2"         "Historically, the general rule for swap partition size was to allocate twice the amount of physical RAM." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-SWAP-PARTITION-INFO-3"         "As computers have gained ever larger memory capacities, this rule has become deprecated." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-SWAP-PARTITION-INFO-4"         "On machines with up to 512MB RAM, the 2x rule is usually adequate." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-SWAP-PARTITION-INFO-5"         "If a sufficient amount of RAM (more than 1024MB) is available, it may be possible to have a smaller swap partition or even eliminate it." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-SWAP-PARTITION-INFO-6"         "With more than 2 GB of physical RAM, one can generally expect good performance without a swap partition." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-SWAP-PARTITION-INFO-7"         "There is always an option to create a swap file after the system is setup." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-SWAP-PARTITION-SWAP"           "Use SWAP"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-SWAP-PARTITION-RAM"            "RAM size is" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-SWAP-PARTITION-RECOMMEND"      "Recommended" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-SWAP-PARTITION-RECOMMEND-MIN"  "Recommended Minimum" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-SWAP-PARTITION-SPECIFY"        "Specify SWAP size as such: 512M or 16G" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-SWAP-PARTITION-SIZE"           "SWAP Size: " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_swap_partition()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-HOME-PARTITION-DESC"      "Get home Partition" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-HOME-PARTITION-TITLE"     "HOME" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-HOME-PARTITION-INFO-1"    "Specify HOME Partition size as such: 512M or 50G" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-HOME-PARTITION-INFO-2"    "Specify 0 for all remaining space on drive, if no more partitions, use 0." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-HOME-PARTITION-SIZE"      "HOME Size: (50G)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-HOME-PARTITION-INFO-3"    "The /home directory stores personal files in different folders." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-HOME-PARTITION-INFO-4"    "It holds miscellaneous personal data as well as user-specific configuration files for applications." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-HOME-PARTITION-INFO-5"    "Keeping it in a separate partition can be very useful for backup: it often requires the most disk space (for desktop users) and may need to be expanded at a later date." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-HOME-PARTITION-INFO-6"    "You can also use HOME on a Separate Drive, in this case pick N, and  choose Y for home on a separate Drive and the below will be commented in your fstab for later editing." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-HOME-PARTITION-PARTITION" "Use home on a separate Partition"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-HOME-PARTITION-DRIVE"     "Use home on a separate Drive"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_home_partition()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-VAR-PARTITION-DESC"         "Get var partition" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-VAR-PARTITION-TITLE"        "var Size" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-VAR-PARTITION-SPECIFY-VAR"  "Specify VAR Partition size as such: Minimum: 8-12 GB" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-VAR-PARTITION-VAR-SIZE"     "VAR Size: (13G)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-VAR-PARTITION-INFO-1"       "The /var directory stores Contains variable data such as spool directories and files, administrative and logging data, pacman's cache, the ABS tree, etc." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-VAR-PARTITION-INFO-2"       "It is used for example for caching and logging, and hence frequently read or written." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-VAR-PARTITION-INFO-3"       "Keeping it in a separate partition avoids running out of disk space due to flunky logs, etc." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-VAR-PARTITION-INFO-4"       "It exists to make it possible to mount /usr as read-only. Everything that historically went into /usr that is written to during system operation (as opposed to installation and software maintenance) must reside under /var." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-VAR-PARTITION-INFO-5"       "You can also use VAR on a Separate Drive, good for Webservers, in this case pick N, and  choose Y for VAR on a separate Drive and the below will be commented in your fstab for later editing." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-VAR-PARTITION-PARTITION"    "Use var on a separate Partition"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-VAR-PARTITION-DRIVE"        "Use var on a separate Drive"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_var_partition()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-TMP-PARTITION-DESC"       "Get tmp partition" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-NOTES"      "@FIX need to fully implement this." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-TMP-PARTITION-TITLE"      "Get tmp partition" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-1"     "By default, a tmpfs partition has its maximum size set to half your total RAM, but this can be customized." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-2"     "Note that the actual memory/swap consumption depends on how much you fill it up, as tmpfs partitions do not consume any memory until it is actually needed." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-3"     "Specify TMP Partition size as such: Current RAM size is" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-4"     "half of that is" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-TMP-SIZE"   "TMP Size: " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-6"     "Directory for programs that require temporary storage of files such as .lck , which can be used to prevent multiple instances of their respective program until a task is completed. Upon completion, the .lck file will be automatically removed." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-7"     "Programs must not assume that any files or directories in /tmp are preserved between invocations of the program and files and directories located under /tmp will typically be deleted whenever the system is booted." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-8"     "tmpfs:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-9"     "tmpfs is a temporary filesystem that resides in memory and/or your swap partition(s), depending on how much you fill it up." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-10"    "Mounting directories as tmpfs can be an effective way of speeding up accesses to their files, or to ensure that their contents are automatically cleared upon reboot." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-11"    "Some directories where tmpfs is commonly used are /tmp, /var/lock and /var/run." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-12"    "Do NOT use it on /var/tmp, because that folder is meant for temporary files that are preserved across reboots." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-13"    "Arch uses a tmpfs /run directory, with /var/run and /var/lock simply existing as symlinks for compatibility." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-14"    "It is also used for /tmp in the default /etc/fstab." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-15"    "By default, a tmpfs partition has its maximum size set to half your total RAM, but this can be customized." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-16"    "Note that the actual memory/swap consumption depends on how much you fill it up, as tmpfs partitions do not consume any memory until it is actually needed." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-17"    "To use tmpfs for /tmp, add this line to /etc/fstab:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-18"    "You may or may not wish to specify the size here, but you should leave the mode option alone in these cases to ensure that they have the correct permissions (1777)." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-19"    "In the example above, /tmp will be set to use up to half of your total RAM. To explicitly set a maximum size, use the size mount option:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-20"    "Here is a more advanced example showing how to add tmpfs mounts for users. This is useful for websites, mysql tmp files, ~/.vim/, and more." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-21"    "It is important to try and get the ideal mount options for what you are trying to accomplish." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-22"    "The goal is to have as secure settings as possible to prevent abuse. Limiting the size, and specifying uid and gid + mode is very secure." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-INFO-23"    "The Default is to use tmpfs." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-tmpfs"      "Use tmp as tmpfs"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-TMP-PARTITION-SIZE-TMPFS" "Set size of tmpfs"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_tmp_partition()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-ROOT-SIZE-DESC"    "Get the Size of the Root" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-ROOT-SIZE-TITLE"   "Get the Size of the Root" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-SIZE-INFO-1"  "The root directory is the top of the hierarchy, the point where the primary filesystem is mounted and from which all other filesystems stem." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-SIZE-INFO-2"  "All files and directories appear under the root directory /, even if they are stored on different physical devices." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-SIZE-INFO-3"  "The contents of the root filesystem must be adequate to boot, restore, recover, and/or repair the system." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-SIZE-INFO-4"  "Therefore, certain directories under / are not candidates for separate partitions." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-SIZE-INFO-5"  "The / partition or root partition is necessary and it is the most important." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-SIZE-INFO-6"  "The other partitions can be replaced by it, even though having different partitions is recommended." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-SIZE-INFO-7"  "Specify ROOT Partition size as such: 512M or 50G" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-SIZE-INFO-8"  "Specify 0 for all remaining space on drive, if no more partitions, use 0." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-SIZE-SIZE"    "ROOT Size: Default is remaining space (0)" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_root_size()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "GET-ROOT-FORMAT-DESC"    "Get the Format of the Root" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "GET-ROOT-FORMAT-TITLE"   "Get the Format of the Root" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-FORMAT-INFO-1"  "The root directory is the top of the hierarchy, the point where the primary filesystem is mounted and from which all other filesystems stem." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-FORMAT-INFO-2"  "All files and directories appear under the root directory /, even if they are stored on different physical devices." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-FORMAT-INFO-3"  "The contents of the root filesystem must be adequate to boot, restore, recover, and/or repair the system." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-FORMAT-INFO-4"  "Therefore, certain directories under / are not candidates for separate partitions." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-FORMAT-INFO-5"  "The / partition or root partition is necessary and it is the most important." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "GET-ROOT-FORMAT-INFO-6"  "The other partitions can be replaced by it, even though having different partitions is recommended." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
get_root_format()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "STATIC-IP-DESC"  "Static IP Address." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "STATIC-IP-NOTES" "Not yet implemented" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    #
    localize_info "STATIC-IP-TITLE"  "Static IP Address" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "STATIC-IP-INFO"   "Available NICs" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "STATIC-IP-NIC-0"  "device eth0 active" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "STATIC-IP-NIC-1"  "device eth1 active" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "STATIC-IP-NIC-2"  "device eth2 active" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "STATIC-IP-STATIC"  "Enter Static IP Address:  [123.123.123.123]: " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CORRECT" "Is this correct"  "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
static_ip()
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "ESCAPE-SED-DESC"  "escape sed" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
escape_sed() 
{
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "ESCAPE-SPECIAL-CHARACTERS-USAGE" "escape_special_characters" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ESCAPE-SPECIAL-CHARACTERS-DESC"  "escape special characters" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "ESCAPE-SPECIAL-CHARACTERS-NOTES" "Not yet Implemented." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
...
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then
    localize_info "CLEAR-ERROR-LOGS-DESC"  "Clear Error Logs" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "CLEAR-ERROR-LOGS-TITLE" "Clearing Error Logs" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
fi
# -------------------------------------
clear_error_logs()
{
...
# -----------------------------------------------------------------------------
if [[ "$RUN_LOCALIZER" -eq 1 ]]; then 
    localize_info "SHOW-CUSTOM-HELP-INFO-1"  "Arch Linux Installation Wizard Script" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-2"  "Arch Linux Installation Wizard AKA archwiz, is an Installation script file to install Arch Linux on a Hard Drive." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-3"  "This script was originally written by helmuthdu[at]gmail[dot]com prior to Nov 2012." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-4"  "It was modified by Jeffrey Scott Flesher: jeff[at]lightwizzard[dot]com" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-5"  "Its written using the Wizard Script API, to make maintaining it easier, and makes the script more robust." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-6"  "It has many Options:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-7"  "To start the install from a Flash Drive to a Hard Drive you wish to Format, do the Following:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-8"  "1.  Boot up Arch Linux ISO and Mount your Flash Drive as such:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-9"  "&nbsp;&nbsp;&nbsp;&nbsp; mkdir usb # You can call it and mount it anywhere you want, but /mnt is not a good place for it; the root is; so /root/usb is good" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-10" "&nbsp;&nbsp;&nbsp;&nbsp; mount /dev/sdb1 usb # where sdb1 is your Flash Drive; assuming you only have to drives in the system; and usb is the folder we created above" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-11" "&nbsp;&nbsp;&nbsp;&nbsp; cd usb" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-12" "&nbsp;&nbsp;&nbsp;&nbsp; Now we are in Boot Mode:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-13" "&nbsp;&nbsp;&nbsp;&nbsp; ./wiz # This is all Menu Driven." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-14" "&nbsp;&nbsp;&nbsp;&nbsp; Note: The Following command will build the Localized Database, this must be done prior to using this script:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-15" "&nbsp;&nbsp;&nbsp;&nbsp; ./wiz -l # Localize" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-16" "&nbsp;&nbsp;&nbsp;&nbsp; Then you must build the Help File if you wish to view it" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-17" "&nbsp;&nbsp;&nbsp;&nbsp; ./wiz -h # Help File for browser: file:///home/USERNAME/Full-Path/archwiz/help.html" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-18" "&nbsp;&nbsp;&nbsp;&nbsp; Automatic install option is -a, it will automatically preform install." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-19" "&nbsp;&nbsp;&nbsp;&nbsp; ./wiz -a # Automatically install." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-20" "&nbsp;&nbsp;&nbsp;&nbsp; Built in self test is -t and -s for special test" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-21" "&nbsp;&nbsp;&nbsp;&nbsp; ./wiz -t # Run all Self-Test" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-22" "&nbsp;&nbsp;&nbsp;&nbsp; ./wiz -s # Run all Special Self-Test" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-23" "2.  Follow the Instructions: These may change and I do not wish to document them here; so a brief overview." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-24" "&nbsp;&nbsp;&nbsp;&nbsp; This Wizard allows you to create a Software List of all Applications you want to install; " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-25" "&nbsp;&nbsp;&nbsp;&nbsp; this list is also known as a script, since you can just run it from a shell terminal," "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-26" "&nbsp;&nbsp;&nbsp;&nbsp; the software list should be created prior to installing the Software," "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-27" "&nbsp;&nbsp;&nbsp;&nbsp; even though it may let you make changes to it after its installed," "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-28" "&nbsp;&nbsp;&nbsp;&nbsp; best practice is to build the system from a fresh install, " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-29" "&nbsp;&nbsp;&nbsp;&nbsp; instead of trying to patch an existing OS," "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-30" "&nbsp;&nbsp;&nbsp;&nbsp; then after a reboot into the new OS created using the Install OS option; " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-31" "&nbsp;&nbsp;&nbsp;&nbsp; it can load the software list using the Load Software option in the menu." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-32" "3.  Reboot; now we are in Live Preinstall Mode:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-33" "&nbsp;&nbsp;&nbsp;&nbsp; cd /home/\$USERNAME" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-34" "&nbsp;&nbsp;&nbsp;&nbsp; mkdir usb # if it does not exist" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-35" "&nbsp;&nbsp;&nbsp;&nbsp; mount /dev/sdb1 usb" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-36" "&nbsp;&nbsp;&nbsp;&nbsp; cd usb" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-37" "&nbsp;&nbsp;&nbsp;&nbsp; ./wiz -a # Automatically install." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-38" "&nbsp;&nbsp;&nbsp;&nbsp; or" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-39" "&nbsp;&nbsp;&nbsp;&nbsp; ./wiz" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-40" "&nbsp;&nbsp;&nbsp;&nbsp; Choose option to Load Software" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-I-40-A"  "&nbsp;&nbsp;&nbsp;&nbsp; After Reboot, you will be in Live Mode." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-41" "Features:" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-42" "Custom Repository: This script creates a Custom Repository with all Software in it;" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-43" "&nbsp;&nbsp;&nbsp;&nbsp; this way it can be installed multiple times and save bandwidth," "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-44" "&nbsp;&nbsp;&nbsp;&nbsp; also it might be possible to install it on a machine with no Internet access," "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-45" "&nbsp;&nbsp;&nbsp;&nbsp; but that has not been tested." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-46" "&nbsp;&nbsp;&nbsp;&nbsp; Currently its limited to just the Core; working on AUR next." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-47" "Profile: This script saves all settings as a Profile per user; it has the ability to change user names; " "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-48" "&nbsp;&nbsp;&nbsp;&nbsp; so you can create a software package installation and customize it to have all the software you want installed on it," "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-49" "&nbsp;&nbsp;&nbsp;&nbsp; and also configure the Hard Drive Partition Scheme to use, while still being able to change both independently of each other," "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-50" "&nbsp;&nbsp;&nbsp;&nbsp; you can copy the profiles to another folder and save them, then simple copy them back; in future versions this will allow" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-51" "&nbsp;&nbsp;&nbsp;&nbsp; you to name the profiles and load them back." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-52" "Optimization: This script allows you to Optimize Pacman to use parallel downloads using Aria2, rsync and ppl parisync, as well as use pacget." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-53" "All downloads are Automatic, unless they fail to install the first time, then they will drop to interactive mode, so you will have to hit keys manually; if AUR package; you will want to say No to edit configuration files, and Yes to all others." "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-54" "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-55" "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-56" "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-57" "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-58" "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
    localize_info "SHOW-CUSTOM-HELP-INFO-59" "" "Comment: FunctionName @ $(basename $BASH_SOURCE) : $LINENO"
